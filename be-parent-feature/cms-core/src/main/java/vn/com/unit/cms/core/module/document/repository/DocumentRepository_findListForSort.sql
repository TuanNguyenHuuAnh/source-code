SELECT 
	...
	, ISNULL(STAA.status_name, (SELECT STATUS_NAME FROM JPM_STATUS_COMMON_LANG WHERE STATUS_COMMON_ID = '1000' AND LANG_CODE = /*searchDto.languageCode*/'VI')) AS status_name
    , ISNULL(STAA.status_code, '000')                       AS status_code
    ...
    ,catedoryDOC.ITEM_FUNCTION_CODE                         AS item_function_code
...
LEFT JOIN M_DOCUMENT_CATEGORY catedoryDOC 
    ON catedoryDOC.ID = TBL.M_CATEGORY_ID		
...
LEFT JOIN JPM_STATUS_DEPLOY sta WITH (NOLOCK)
	ON sta.ID = step.STATUS_DEPLOY_ID
	
LEFT JOIN JPM_STATUS_LANG_DEPLOY statusLang WITH (NOLOCK)
	ON statusLang.STATUS_DEPLOY_ID = sta.ID
...
OUTER APPLY dbo.[FN_GET_STATUS_PROCESS](tbl.DOC_ID, /*searchDto.languageCode*/'VI') STAA
WHERE
  ...
  AND
	...
	AND tbl.M_CATEGORY_ID =  /*searchDto.categoryId*/''
