...
import vn.com.unit.ep2p.admin.validators.PopupValidator;
...
public class PopupController {
    ...
    private static final String TEMPLATE_VIEW = "/views/popup/popup-list.html";

    private static final String TEMPLATE_TABLE = "/views/popup/popup-table.html";

    private static final String TEMPLATE_DETAIL = "/views/popup/popup-detail2.html";

    private static final String TEMPLATE_EDIT = "/views/popup/popup-edit.html";
...
//    public static final String EMAIL_TEMPALTE_FOLDER = "email_template/";
    ...
    public ModelAndView listTemplate(@ModelAttribute(value = "searchDto") PopupSearchDto searchDto,
     ...
     // Session Search
        ConditionSearchUtils<PopupSearchDto> searchUtil = new ConditionSearchUtils<PopupSearchDto>();
        String[] urlContains = new String[] { "popup/add", "popup/edit", "popup/detail", "popup/list" };
        ...
        PageWrapper<PopupDto> pageWrapper = popupService.doSearch(page, searchDto, pageSize, locale);
    ...
    @ResponseBody
    public ModelAndView ajaxListEmail(@ModelAttribute(value = "searchDto") PopupSearchDto searchDto,
            @RequestParam(value = ConstantCore.PAGE_SIZE) Optional<Integer> pageSizeParam,
            @RequestParam(value = "page", required = false, defaultValue = "1") int page, Locale locale, HttpServletRequest request)
            throws DetailException {
    ...
    public ModelAndView postDelete(@RequestParam(value = "templateId", required = true) Long templateId, Locale locale,
        ...
        try {
            popupService.deleteTemplate(templateId);
        } catch (Exception e) {
        ...
        // redirect list page
        String viewName = UrlConst.REDIRECT.concat(UrlConst.POPUP).concat(UrlConst.LIST);
    ...
    public ModelAndView getEdit(@RequestParam(value = "id", required = false) Long id, Model model, Locale locale) {
        ...
        PopupDto objectDto = new PopupDto();
        // set url ajax
        String url = UrlConst.POPUP.concat(UrlConst.EDIT);
        if (null != id) {
            objectDto = popupService.getPopupDtoById(id);
        ...
        // init data
        popupService.InitScreenEdit(mav, objectDto, locale);
    ...
    public ModelAndView postEdit(@Valid @ModelAttribute(value = OBJECT_DTO) PopupDto objectDto,
...
//        }
        ...
        popupValidator.validate(objectDto, bindingResult);
        if (!bindingResult.hasErrors()) {
            try {
                PopupDto result = popupService.savePopup(objectDto);
                messageList.setStatus(Message.SUCCESS);
                msgInfo = msg.getMessage(ConstantCore.MSG_SUCCESS_SAVE, null, locale);
                messageList.add(msgInfo);
                String viewName = UrlConst.REDIRECT.concat(UrlConst.POPUP).concat(UrlConst.EDIT);
    ...
    public JcaEmailTemplate getContentTemplate(@RequestParam("templateId") String templateId) {
        JcaEmailTemplate template = popupService.getTemplateById(Long.parseLong(templateId));
        return template;
    }
