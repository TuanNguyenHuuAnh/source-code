/*******************************************************************************
 * Class        ：PushNotificationMobileJob
...
import vn.com.unit.ep2p.admin.service.PushNotificationJobService;
...
public class PushNotificationMobileJob implements Job{
    ...
    @Autowired
    private PushNotificationJobService pushNotificationJobService;
    
    private static final Logger logger = LoggerFactory.getLogger(PushNotificationMobileJob.class);
    ...
    public void execute(JobExecutionContext context){
        ...
        try {
            ...
            logger.info("##PushNotificationMobileJob - before excute job (QrtzMJobLog) : " + JsonUtils.convertObjectToJSON(jobLog));
            jobLog = jobLogService.save(jobLog);

            jobScheduleService.updateScheduleJob(jobSchedule, JobStatus.RUNNING, ValidFlagEnum.ACTIVED);

            int count = pushNotificationJobService.scanTableAndPushNotif(SecurityContextHolder.getContext(), companyId);
            logger.debug("## == push Notification a total of "+ count +" message(s) == ##");
            ...
            // Log error
            logger.error("##PushNotificationMobileJob", e);

        } finally {
            jobLog.setEndTime(endTime);
            jobLog = jobLogService.save(jobLog);
            try {
                logger.info("##PushNotificationMobileJob - before excute job (QrtzMJobSchedule) : " + JsonUtils.convertObjectToJSON(jobSchedule));
            } catch (Exception e) {
                logger.error("##PushNotificationMobileJob", e);
            }
            jobScheduleService.save(jobSchedule);
