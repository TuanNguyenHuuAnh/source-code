...
FROM
	...
	INNER JOIN JCA_ROLE_FOR_ACCOUNT RFA ON ROLE.ID = RFA.ROLE_ID
	INNER JOIN JCA_ACCOUNT ACCOUNT ON ACCOUNT.ID = RFA.ACCOUNT_ID
	LEFT JOIN JCA_M_STATUS_AUTHORITY SA ON (
		ITEM.FUNCTION_TYPE = '1' 
		AND ITEM.BUSINESS_CODE IS NOT NULL 
		AND ROLE.ID = SA.ROLE_ID 
		AND SA.ITEM_ID = ITEM.ID 
	)
...
WHERE 
	/*currentDate*/ BETWEEN RFA.START_DATE AND RFA.END_DATE
