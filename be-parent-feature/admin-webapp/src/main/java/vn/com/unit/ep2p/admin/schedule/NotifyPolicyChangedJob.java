...
public class NotifyPolicyChangedJob implements Job {
    ...
    @Autowired
    NotifyCommonService notifyService;
    
    private static final Logger logger = LoggerFactory.getLogger(NotifyPolicyChangedJob.class);
    ...
    public void execute(JobExecutionContext context) {
        ...
        try {
            ...
            // ------push notify------
            try {
            	List<PolicyChangedDto> listPolicyChanged = db2ApiService.getPolicyChanged();
            	for (PolicyChangedDto dto : listPolicyChanged) {
            		notifyService.pushNotifyForWeb(dto.getAgentCode(), dto.getTitle(), dto.getContents(), dto.getPolicyNo());
            		notifyService.pushNotifyForWeb(dto.getAgentCodeDuo(), dto.getTitle(), dto.getContents(), dto.getPolicyNo());
					if ("AG".equals(dto.getChannel())) {
						this.pushNotificationToFirebaseCloud(dto);
					} else if ("AD".equals(dto.getChannel())) {
						this.pushNotificationForADPortal(dto);
					}
				}
            } catch (Exception e) {
            ...
            System.out.println("OrderCountdownJob run at: " + CommonDateUtil.formatDateToString(new Date(), CommonDateUtil.YYYYMMDDHHMMSSFFF));
            ...
            // Log error
            logger.error("## OrderCountdownJob ##", e);
    ...
    private void pushNotificationToFirebaseCloud(PolicyChangedDto dto) throws Exception {
    	try {
            ...
            try {
                ...
                if (dto.getAgentCode() != null) {
					List<JcaAccount> lstAccount = jcaAccountService.getListByUserNameList(dto.getAgentCode());
					...
					for (JcaAccount acc : lstAccount) {
 					...
 					MulticastMessage multicastMessage = allPlatformsMessage(lstToken, dto.getTitle(), dto.getContents());
 					...
 					if (response.getFailureCount() > 0) {
 						...
 						if (!failedTokens.isEmpty()) {
 					        systemLogsService.writeSystemLogs("NOTIFY_CODE_ORDER", "Send notif device", "List of tokens that caused failures: " + String.join(",", failedTokens), null);
 		        ...
 		        systemLogsService.writeSystemLogs("NOTIFY_CODE_ORDER", "Send notif device", ex.getMessage(), null);
                 logger.error("PUSH_NOTIFICATION", ex);
             }
         } catch (Exception e) {
             logger.error(e.getMessage(), e);
 	        systemLogsService.writeSystemLogs("NOTIFY_CODE_ORDER", "Send notif device", e.getMessage(), null);
             throw new Exception("Push notification to firebase failed!");
         }
     }

    public MulticastMessage allPlatformsMessage(List<String> deviceToken, String title, String content) {
    ...
    private void pushNotificationForADPortal(PolicyChangedDto dto) throws Exception {
    	try {
    		List<String> lstAgentCode = Arrays.asList(dto.getAgentCode(), dto.getAgentCodeDuo());
    		List<ADPDeviceTokenDto> deviceToken = db2ApiService.getDeviceTokenInfo(lstAgentCode);
    		if (deviceToken != null && deviceToken.size() > 0) {
    			notifyService.insertNotify(deviceToken, dto.getContents(), dto.getPolicyNo());	
    		}
    	} catch (Exception e) {
    		logger.error(e.getMessage(), e);
  	        throw new Exception("Push notification to mADP failed!");
  	    }
    }
}
