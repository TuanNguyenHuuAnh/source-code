...
public class NotifyBaseServiceImpl implements NotifyBaseService {
	...
	public Long pushListNotifyForWeb(List<NotifyBaseDto> notifications) {
		Long id = 0L;
		for (NotifyBaseDto notify : notifications) {
			if (id == 0L) {
				//save notify
				NotifyAdmin entity = new NotifyAdmin();
				entity.setNotifyCode(getNotifyCode());
				entity.setNotifyTitle(notify.getTitle());
				entity.setNotifyType(2);
				entity.setContents(notify.getContents());
				...
				SystemOutLogUtils.printLogObjectDto("## NotifyAdmin ## DTO=", entity);
			...
			entityDetail.setAgentCode(new Long(notify.getAgentCode()));
			entityDetail.setReadAlready(false);
			entityDetail.setCreateDate(new Date());
			SystemOutLogUtils.printLogObjectDto("## NotifyDetailAdminEditDto ## DTO=", entityDetail);
