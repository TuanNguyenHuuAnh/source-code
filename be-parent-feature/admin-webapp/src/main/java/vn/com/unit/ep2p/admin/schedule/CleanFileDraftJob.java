/*******************************************************************************
 * Class        ：CleanFileDraftJob
 * Created date ：2019/09/26
 * Lasted date  ：2019/09/26
 * Author       ：KhuongTH
 * Change log   ：2019/09/26：01-00 KhuongTH create a new
 ******************************************************************************/
...
public class CleanFileDraftJob implements Job {
...
//    @Autowired
//    private OZDocMainFileService ozDocMainFileService;

    private static final Logger logger = LoggerFactory.getLogger(CleanFileDraftJob.class);
    ...
    public void execute(JobExecutionContext context){
        ...
        try {
           ...
           /* systemLogsService.writeSystemLogs(CommonConstant.SC1_S02_JOBSCHEDULE, job.getJobName() + " start", LogTypeEnum.INFO.toInt(),
            ...
            logger.info("##CleanFileDraftJob - before excute job (QrtzMJobLog) : " + JsonUtils.convertObjectToJSON(jobLog));
            jobLog = jobLogService.save(jobLog);

            jobScheduleService.updateScheduleJob(jobSchedule, JobStatus.RUNNING, ValidFlagEnum.ACTIVED);

//            int totalDeleted = ozDocMainFileService.deleteDraftFile();
//            logger.info("## == deleted "+ totalDeleted +" file draft == ##");
            ...
            // Log error
            logger.error("##CleanFileDraftJob", e);

        } finally {
            jobLog.setEndTime(endTime);
            jobLog = jobLogService.save(jobLog);
            try {
                logger.info("##CleanFileDraftJob - before excute job (QrtzMJobSchedule) : " + JsonUtils.convertObjectToJSON(jobSchedule));
            } catch (Exception e) {
                logger.error("##CleanFileDraftJob", e);
            }
            jobScheduleService.save(jobSchedule);
