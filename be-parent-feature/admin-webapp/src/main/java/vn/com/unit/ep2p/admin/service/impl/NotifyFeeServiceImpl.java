...
public class NotifyFeeServiceImpl implements NotifyFeeService {
	...
	public Long pushListNotifyForWeb(List<NotifeeDto> notifees) {
		Long id = 0L;
		for (NotifeeDto notifee : notifees) {
			if (id == 0L) {
				//save notify
				NotifyAdmin entity = new NotifyAdmin();
				entity.setNotifyCode(getNotifyCode());
				entity.setNotifyTitle(notifee.getTitle());
				entity.setNotifyType(2);
				entity.setContents(notifee.getContents());
			...
			entityDetail.setAgentCode(new Long(notifee.getAgentCode()));
