$(document).ready(function($) {
	...
	*/
	...
	$("#addNew").on("click", function(event) {
		...
		// Redirect to page adds
		ajaxRedirectWithCondition(url, setConditionSearch());
	});
	$('#exportGeneral').click(function() {
		let linkExport = BASE_URL + PAGE_URL + "/export-general";
		//		 condition
	...
	$("#exportDetail").click(function() {
		let linkExport = BASE_URL + PAGE_URL + "/export-detail";
		//		 condition
		var condition = setConditionSearch();
		//		 do export excel
		doExportFileExcel(linkExport, "token", condition);
...
function getValueByConditionSearch(value, conditionSearch){
	...
	if (isNull(conditionSearch)) {
		...
		if (conditionSearch.indexOf('LIKE') > -1) {
			...
			if (!isNull(value)){
				if(isNaN(value) && value.indexOf('CONVERT(DATE,') > -1){
