...
*/	
    ...
    if (SEARCH_DTO.searchType == 1){ // search field
        $("#tableList").datatables({
            url : BASE_URL + 'banner/homepage-setting/ajaxList',
            type: 'POST',
	...
	$("#addNew").unbind().bind("click", function(event) {
		var url = BASE_URL + "banner/homepage-setting/edit";
	...
	*/
	$('#btnSort').on('click', function(event) {
		let url = BASE_URL +"banner/homepage-setting/list/sort";
...
function editBanner(element, event, row) {
	...
	var url = BASE_URL + "banner/homepage-setting/edit?id=" + id;
...
function deleteBanner(element, event) {
	...
	popupConfirmWithButtons(MSG_DEL_CONFIRM, LIST_BUTTON_CONFIRM_DELETE, function(result) {
		if (result) {
			var url = "banner/homepage-setting/delete";
			var condition = {
				"id" : bannerId
			}
...
function detail(element, event) {
	...
	var row = $(element).parents("tr");
	var id = row.data("banner-id");
	var url = BASE_URL + "banner/homepage-setting/detail?id=" + id;
...
function onClickSearch(element, event) {
	...
	if ($("#mBannerTopId").is(':checked')){
		if ($("#fieldValuesHidd").val() == null || $("#fieldValuesHidd").val() == ""){
			$("#fieldValuesHidd").val($("#mBannerTopId").val());
		}else{
			$("#fieldValuesHidd").val($("#mBannerTopId").val() + "," + $("#fieldValuesHidd").val());
		}
	}
	
	if ($("#mBannerTopMobileId").is(':checked')){
		if ($("#fieldValuesHidd").val() == null || $("#fieldValuesHidd").val() == ""){
			$("#fieldValuesHidd").val($("#mBannerTopMobileId").val());
		}else{
			$("#fieldValuesHidd").val($("#mBannerTopMobileId").val() + "," + $("#fieldValuesHidd").val());
		}
	}
		
	ajaxSearch("banner/homepage-setting/ajaxList", setConditionSearch(), "tableList", element, event);
}
...
function onClickSearchAll(element, event) {
	...
	ajaxSearch("banner/homepage-setting/ajaxList", condition, "tableList", element, event);
}
