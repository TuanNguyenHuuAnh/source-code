    var CONTROLLER_URL = 'emulate-result';
...
$(function(){
    ...
    $('#btnDelete').unbind().bind('click', function(e){
        if (!$(this).hasClass('not-active')){
            popupConfirm(MSG_CONFIRM_DELETE, function(result){
                if (result){
                    let url = CONTROLLER_URL + "/delete-movement";
                    // ajaxSubmit(url, setConditionSearchImport(), e);
                    ajaxSubmitCallBack(url, setConditionSearchImport(), e, notActive);
                }
            })
        }
    })
