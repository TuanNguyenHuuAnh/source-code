...
$(document).ready(function () {
    ...
    searchCombobox('.service-function', '', 'jpm-svc-management/get-item',
    	    function data(params) {
    	        var obj = {
    	            keySearch: params.term,
    	            companyId: $(this).parents(".register-form").find('.company-id').val(),
    	            businessId: $(this).parents(".register-form").find('#businessCode').val(),
    	            isPaging: true,
    	            mode: 1
    	        };
    	        return obj;
    ...
    //onchange business
    $('.service-business').on('change.select2', function() {
...
function defaultFunctionByBusiness(element){
	...
	var form = $(element).parents(".register-form");
	var companyId = $(form).find('.company-id').val();
	$.ajax({
		type : "POST",
		url : BASE_URL + 'jpm-svc-management/get-item-default',
		data : {companyId: companyId, businessId: businessId},
		success : function(data) {
			$(form).find('.service-function').prop('disabled', false);
			$(form).find('#functionCodeIp').prop('disabled', true);
			if(null != data.id){
				var newOption = new Option(data.text, data.id, false, true);
				$(form).find('.service-function').append(newOption).trigger('change');
			}
			if(data.name == "2"){
				$(form).find('.service-function').prop('disabled', true);
				$(form).find('#functionCodeIp').val(data.id);
				$(form).find('#functionCodeIp').prop('disabled', false);
			}
		},
		error : function(xhr, textStatus, error) {
			console.log(xhr);
