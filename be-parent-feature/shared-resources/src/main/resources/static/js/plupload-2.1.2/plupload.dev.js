...
//config filter: min_width_min_height
plupload.addFileFilter('min_width_x_min_height', function(minSize, file, cb) {
    var self = this, img = new o.Image();
 
    function finalizeImg(result) {
        // cleanup
        img.destroy();
        img = null; 
        ...
        if (!result) {
          self.trigger('Error', {
            code : 401,
            message : "Min size width and height is " + minSize.width  + "x" + minSize.height,
            file : file
          });
    ...
    img.onload = function() {
        if (minSize != null) {  
            ...
            finalizeImg((img.width >= minSize.width) && (img.height >= minSize.height));
        } else {
            finalizeImg(true);
        }
    };
