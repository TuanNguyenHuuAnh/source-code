...
function onClickSearch(element, event) {
	...
	setDataSearchHidden();
	ajaxSearch("email/template/ajaxList", setConditionSearch(), "tableList", element, event);
}
...
function saveTemplate(){
	...
	var code = $('#code').val();
	var ajax = $.ajax({
        url: BASE_URL + "email/template/save-template",
        type: "POST",
        data : {templateId : templateId,
        		content : content,
        		fileName : fileName,
...
function getDetailByteOut(templateId, $this){
	$.ajax({
        url: BASE_URL + "email/template/template-email-detail",
        type: "POST",
        data : {templateId : templateId},
        global : false
    })
...
function deleteTemplate(templateId){
	popupConfirm(MSG_DEL_CONFIRM, function (result) {
	    if (result) {
			...
			ajaxSubmit("email/template/delete", {templateId : templateId}, event);
	...
	$('#uploadModal').fileupload({
	    ...
	    add: function (e, data) {
	    	...
	    	$('#btnImportModal').unbind('click').bind('click',function(e) {
	    		...
	    		if(data.files != null){
	    			data.url= BASE_URL + 'email/template/import-modal';
	    			data.submit();
	    			data.files = null;
	    			$("#fileNameModal").html('');
	    		}
...
function checkParam(){
	...
	$.ajax({
        url: BASE_URL + "email/template/check-params",
        type: "POST",
        data :{
...
function editTemplate(id) {
	event.preventDefault();
	var url = BASE_URL + "email/template/edit?id=" + id;
	// Redirect to page detail
	ajaxRedirect(url);
}
...
function detailTemplate(id) {
	event.preventDefault();
	var url = BASE_URL + "email/template/detail?id=" + id;
	// Redirect to page detail
	ajaxRedirect(url);
}
