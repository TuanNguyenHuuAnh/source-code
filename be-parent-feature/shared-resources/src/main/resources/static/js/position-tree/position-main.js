$(document).ready(function() {
	...
	$('#id-organization-tree').tree({
	    url: BASE_URL + 'position-tree/get-node'
...
function getOrgDetail(node) {
	...
	$.ajax({
        type  : "GET",
        url   : BASE_URL + "position-tree/detail",
        data  : {
        	"orgId" : node.id
        },
        ...
        complete:function(){
        	if(node.state == 'closed' || node.children!=null)
        	{
        		$('#id-btn-delete-org').addClass('hide');
        	}
        	loadUserList(node.id);
        	if(messageList!=null){
        		$('#msg').html('<div class="alert alert-${messageList.messages[0].status} alert-dismissible"><button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button><h4><i class="icon fa fa-${messageList.messages[0].status}"></i>Alert!</h4><div/>${messageList.messages[0].content}</div>');
        		messageList = null;
        	}
        }
    });
...
function loadOrgTree(nodeId, selectId) {
	...
	$.ajax({                
    	dataType : "json",
        type  : "POST",
        url   : BASE_URL + "position-tree/build-organization",
...
function loadListAccDep(orgId) {
	$.ajax({
        type  : "GET",
        url   : BASE_URL + "position-tree/view-account-list",
        data  : {
        	"orgId" : orgId
        },
        success: function (data , textStatus, resp ) {
...
function edit() {
	$.ajax({
        type  : "GET",
        url   : BASE_URL + "position-tree/edit",
        data  : {
...
function newOrg() {
	$.ajax({
        type  : "GET",
        url   : BASE_URL + "position-tree/add",
        data  : {
        	"orgId" : $('#id-current-org-id').val()
        },
        success: function (data , textStatus, resp ) {
...
function update() {
	if ($(".j-form-validate").valid()) {
		...
		$.ajax({                	        	
            type  : "POST",
            data  : values,
            url   : BASE_URL + "position-tree/update",
            success: function (data) {	          
            	$("#id-box-org-detail").html(data);	            	         
            },
            complete:function()
...
function deleteOrg() {
	popupConfirm( MSG_DEL_CONFIRM, function(result) {
		if (result) {
			$.ajax({
		        type  : "POST",
		        url   : BASE_URL + 'position-tree/delete',
		        data  : {
		        	"orgId" : $('#id-current-org-id').val()
		        },
		        success: function (data) {
		        	var status = data.messages[0].status;
		        	...
		        	if(status=='success'){
		        		...
		        		$('#msg').html('<div class="alert alert-'+status+' alert-dismissible"><button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button><h4><i class="icon fa fa-'+status+'"></i>Alert!</h4><div/>'+content+'</div>');
		        	}
...
function loadUserList(orgId){
	ajaxSearch("position-tree/ajaxList", {"orgId" : orgId}, "userList", this, event);
}
