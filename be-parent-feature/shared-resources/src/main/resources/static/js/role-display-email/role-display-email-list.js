$(document).ready(function() {
 ...
 function ajaxLoadCompanyByRole(rowRoleSelected){
	...
	$.ajax({
	      ...
	      url   : BASE_URL + "role-display-email/get-list-company-for-role",
	      data  : {
	    	  "role_id" : roleId
	      },
 ...
 function ajaxSaveAuthority(){
	...
	if(rowRoleSelected.length > 0) {
		...
		$.ajax({
		      type  : 'POST',	      
		      url   : BASE_URL + "role-display-email/post-list-company-for-role",
 ...
 function getAuthorityJson() {
	 ...
	 $('#companyTableId tbody tr').each(function(key, val) {
		 authority.push({
			 "id" 					:   $("#id" + key).val(),
			 "companyId" 			:   parseInt($("#companyId" + key).val()),			
			 "flgChecked" 			:	true,
			 "orgId" 			 	:   $("#listOrg" + key).combotree('getText')== ""? null : parseInt($("#listOrg" + key).combotree('getValue')),
		  });
	 });
 ...
 function ajaxLoadAuthority(companyId) {
		$.ajax({
			  async : false,
		      type  : "GET",
		      url   : BASE_URL + "role-display-email/ajax/list",
		      data  : {
		    	  "companyId" : companyId
		      },
		      success: function( data ) {
