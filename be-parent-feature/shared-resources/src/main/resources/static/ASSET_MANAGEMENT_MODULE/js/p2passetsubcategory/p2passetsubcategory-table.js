...
$(function(){
	...
	$("#tableList").datatables({
		url : BASE_URL + 'asset-sub-category/ajaxList',
		type : 'POST',
	...
	$('#btnExport').click(function() {
		var linkExport = BASE_URL + "asset-sub-category/export-excel";
...
function onClickSearch(element, event) {
	setDataSearchHidden();
	ajaxSearch("asset-sub-category/ajaxList", setConditionSearch(), "tableList", element, event);
}
...
function editItem(element, event) {
	...
	var id = row.data("id");
	var url = BASE_URL + "asset-sub-category/edit?id=" + id;
...
function deleteItem(element, event){
	...
	popupConfirm( MSG_DEL_CONFIRM, function(result) {
		if (result) {
			var url = "asset-sub-category/delete";
			var condition = {
				"id" : id
			}
...
function detailItem(element, event) {
	...
	var row = $(element).parents("tr");
	var id = row.data("id");
	var url = BASE_URL + "asset-sub-category/detail?id=" + id;
