$(function(){
	...
	$("#tableList").datatables({
		url : BASE_URL + 'office-location/ajaxList',
		type : 'POST',
	...
	$("#btnExport").unbind('click').bind('click', function(){
		var linkExport = BASE_URL + "office-location/export-excel";
...
function onClickSearch(element, event) {
	setDataSearchHidden();
	ajaxSearch("office-location/ajaxList", setConditionSearch(), "tableList", element, event);
}
...
function editItem(element, event) {
	...
	var id = row.data("id");
	var url = BASE_URL + "office-location/edit?id=" + id;
...
function deleteItem(element, event){
	...
	popupConfirm( MSG_DEL_CONFIRM, function(result) {
		if (result) {
			var url = "office-location/delete";
			var condition = {
				"id" : id
			}
...
function detailItem(element, event) {
	...
	var row = $(element).parents("tr");
	var id = row.data("id");
	var url = BASE_URL + "office-location/detail?id=" + id;
