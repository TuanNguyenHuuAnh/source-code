...
function addPrToAssetReturn(hashMap) {
...
function addPrToAsset(value, key, map) {
	...
	if (value) {
		// check id da ton tai trong ds ben duoi chua, neu chua ton tai thi cho copy xuong
		if ($("#pr_" + key).length == 0) {
			// get Max index
			...
			var propertyOf = $row.data("propertyof");
			if (propertyOf == undefined) {
				propertyOf = '';
			}
			...
			content = '<tr class="row_pr align-top" id="pr_'
					...
					+ ' </td>' + '  <td>' + assetStatusName + ' </td>' + '  <td>'
					+ serialNo + ' </td>' + '  <td>' + assetTag + ' </td>'
