WITH stepStatus AS (
	...
	LEFT JOIN JPR_PROCESS_STATUS_LANG STATUSLANG
	        ...
	        AND UPPER(statusLang.LANG_CODE) = UPPER(/*condition.languageCode*/'VI')
)

SELECT
	ROW_NUMBER() OVER (ORDER BY m_intro_category.sort ASC, m_intro_category.create_date DESC, m_intro_category.code ASC) AS stt
	   , m_intro_category.code
	   , m_language.label						AS	title
	   , CASE WHEN m_intro_category.picture_introduction = 1 THEN 'x'
			ELSE  ''
		END  									picture_introduction
	   , CASE WHEN m_intro_category.type_of_main = 1 THEN 'x'
			ELSE  ''
		END  									type_of_main
	   , CASE WHEN m_intro_category.enabled = 1 THEN 'x'
			ELSE  ''
		END  									enabled
	   , CASE WHEN m_intro_category.status = 1 THEN /*condition.statusName*/'Lưu nháp'
			ELSE  stepStatus.STATUS_NAME
		END  									AS status_name
		, m_intro_category.create_by			AS create_by
		, m_intro_category.create_date			AS create_date
	FROM
	...
	LEFT JOIN m_introduction_category_language m_language ON (m_language.m_introduce_category_id = m_intro_category.id
	    AND m_language.m_language_code = UPPER(/*condition.languageCode*/)
