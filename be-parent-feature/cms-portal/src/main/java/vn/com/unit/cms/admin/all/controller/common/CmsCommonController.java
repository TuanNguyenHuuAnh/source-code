...
/**
 * @author TaiTM
 * @description Controller common giành cho CMS không sử dụng customerAlias
 */
public abstract class CmsCommonController<SEARCH_DTO extends CmsCommonSearchFilterDto, SEARCH_RESULT_DTO extends CmsCommonSearchResultFilterDto, EDIT_DTO extends CmsCommonEditDto> {
     ...
     */
    public void initScreenListSort(ModelAndView mav, SEARCH_DTO searchDto, Locale locale) {
     ...
     */
    abstract public String viewListSort();
     ...
     */
    abstract public String viewListSortTable();
     ...
     */
    abstract public String viewEdit();
     ...
     * @example "/example"
     */
    abstract public String getUrlByAlias();
     ...
     */
    abstract public boolean hasRoleForList();
     ...
     */
    abstract public boolean hasRoleForEdit();
     ...
     */
    abstract public boolean hasRoleForDetail();
    ...
    public ModelAndView doGetList(@ModelAttribute SEARCH_DTO searchDto,
        ...
        String[] urlContains = new String[] { getUrlByAlias()+"/add", getUrlByAlias()+"/edit", getUrlByAlias()+"/detail", getUrlByAlias()+"/list" };
        ...
        mav.addObject("controllerUrl", getUrlByAlias());
        mav.addObject("functionCode", getFunctionCode());
        mav.addObject("hasRoleForEdit", hasRoleForEdit());
        mav.addObject("hasRoleForDetail", hasRoleForDetail());
        mav.addObject("hasRoleForDelete", hasRoleForDelete());
        mav.addObject("hasRoleForListSort", hasRoleForListSort());
        mav.addObject("titlePageList", getTitlePageList(locale));
        mav.addObject("showExportExcel", hasExportExcel());
        mav.addObject("hasRoleForImport", showButtonImport());
    ...
    @ResponseBody
    public ModelAndView ajaxList(@ModelAttribute SEARCH_DTO searchDto,
            @RequestParam(value = ConstantCore.PAGE_SIZE) Optional<Integer> pageSizeParam,
            @RequestParam(value = ConstantCore.PAGE) Optional<Integer> pageParam, Pageable pageable,
            HttpServletRequest request, Locale locale) throws DetailException {
        ...
        ModelAndView mav = new ModelAndView(viewListTable());
     ...
     */
    protected EDIT_DTO getData(Long id, SEARCH_DTO condition, String searchDtoJson, boolean isEdit, Locale locale) {
        // Get Edit dto
    ...
    public ModelAndView doGetDetail(@ModelAttribute(value = "searchDto") SEARCH_DTO condition,
        ...
        // Start get data
        EDIT_DTO editDto = getData(id, condition, searchDtoJson, false, locale);
    ...
    public ModelAndView doGetEdit(@ModelAttribute(value = "searchDto") SEARCH_DTO condition,
            @RequestParam(value = "searchDtoJson", required = false) String searchDtoJson, Locale locale,
            @RequestParam(value = "id", required = false) Long id) {
        ModelAndView mav = new ModelAndView(viewEdit());
        ...
        // Start get data
        EDIT_DTO editDto = getData(id, condition, searchDtoJson, true, locale);
        mav.addObject(mavObjectEditName(), editDto);
        // End get data
     ...
     */
    protected ModelAndView returnError(EDIT_DTO editDto, Locale locale) {
        ModelAndView mav = new ModelAndView(viewEdit());
    ...
    public ModelAndView doSubmit(@ModelAttribute EDIT_DTO condition, BindingResult bindingResult, Locale locale,
        ...
        try {
            ...
            if (StringUtils.isBlank(condition.getCode())) {
                if (getPrefixCode() != null) {
                    condition.setCode(CommonUtil.getNextCode(getPrefixCode(),
                            cmsCommonService.getMaxCode(getTableForGenCode(), getPrefixCode())));
                }
            }
    ...
    public ModelAndView doDeleteData(@ModelAttribute SEARCH_DTO searchDto,@RequestParam(value = "id", required = true) Long id, Locale locale,
            RedirectAttributes redirectAttributes) {
        ...
        String viewName = UrlConst.REDIRECT.concat(UrlConst.ROOT).concat(getUrlByAlias()).concat(UrlConst.LIST);
    ...
    public ModelAndView doGetListSort(@ModelAttribute SEARCH_DTO searchDto, Locale locale,
            @RequestParam(value = ConstantCore.PAGE, required = false, defaultValue = "1") int page) {

        // Start check role
        if (!hasRoleForListSort()) {
            return new ModelAndView(ViewConstant.ACCESS_DENIED_MODELANDVIEW);
        }
        // End check role

        ModelAndView mav = new ModelAndView(viewListSort());
    ...
    public ModelAndView ajaxListSort(@ModelAttribute SEARCH_DTO condition, Locale locale) {

        ModelAndView mav = new ModelAndView(viewListSortTable());

        // set language
        condition.setLanguageCode(locale.toString());

        // Start do search
        List<SEARCH_RESULT_DTO> list = getCmsCommonSearchFillterService().getListForSort(condition);
        mav.addObject("sortPageModel", list);
        // End do search
    ...
    @ResponseBody
    public ModelAndView doSaveListSort(@RequestBody SEARCH_DTO searchDto, Locale locale,
            RedirectAttributes redirectAttributes) {
        // Start check role
        if (!hasRoleForEdit()) {
            return new ModelAndView(ViewConstant.ACCESS_DENIED_MODELANDVIEW);
    ...
    public @ResponseBody String editorUpload(MultipartHttpServletRequest request, HttpServletRequest request2,
        ...
        String downloadUrl = request2.getContextPath().concat(UrlConst.ROOT).concat(getUrlByAlias())
    ...
    public void exportExcel(@ModelAttribute SEARCH_DTO searchDto, Pageable pageable, Locale locale,
