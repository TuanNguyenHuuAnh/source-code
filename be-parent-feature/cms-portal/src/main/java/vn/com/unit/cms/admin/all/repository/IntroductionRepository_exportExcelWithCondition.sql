WITH stepStatus AS (
	...
	LEFT JOIN JPR_PROCESS_STATUS_LANG STATUSLANG
	        ...
	        AND UPPER(statusLang.LANG_CODE) = UPPER(/*condition.languageCode*/'')
)
SELECT
	  ROW_NUMBER() OVER (ORDER BY m_category_lang.LABEL, m_intro.sort, m_intro.create_date DESC, m_intro.code ASC) AS stt,
	  m_intro.code			AS code,
	  m_intro.create_by		AS create_by,
	  m_intro.create_date	AS create_date,
	  ...
	  , CASE WHEN m_intro.status = 1 THEN /*condition.statusName*/'Lưu nháp'
		...
		END  									AS STATUS_NAME
	  , CASE WHEN m_intro.enabled = 1 THEN 'x'
			ELSE  ''
		END  									AS enabled
...
LEFT JOIN m_introduction_category_language m_category_lang
	ON(m_category_lang.M_INTRODUCE_CATEGORY_ID = m_category.id
		...
		AND UPPER(m_category_lang.M_LANGUAGE_CODE) = UPPER(/*condition.languageCode*/)
	)
LEFT JOIN m_introduction_language m_language 
	ON (m_language.m_introduction_id = m_intro.id 
	    AND UPPER(m_language.m_language_code) = UPPER(/*condition.languageCode*/)
...
WHERE 
	...
	AND m_intro.name LIKE concat('%',/*condition.name*/,'%')
	/*END*/
