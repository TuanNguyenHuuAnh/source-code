...
WHERE
	...
	AND (m_role.company_id is null or m_role.company_id = /*roleSearchDto.companyId*/1)
	/*END*/
	AND m_role.role_type = '1'
	/*BEGIN*/
	AND(
		/*IF roleSearchDto.code != null && roleSearchDto.code != ''*/
		OR replace(m_role.code,' ','') LIKE CONCAT( '%', CONCAT(/*roleSearchDto.code*/, '%') )
		/*END*/
		
		/*IF roleSearchDto.name != null && roleSearchDto.name != ''*/
		OR replace(m_role.name,' ','') LIKE CONCAT( '%', CONCAT(/*roleSearchDto.name*/, '%') )
		/*END*/
		
	    /*IF roleSearchDto.description != null && roleSearchDto.description != ''*/
        OR replace(m_role.description,' ','') LIKE CONCAT( '%', CONCAT(/*roleSearchDto.description*/, '%') )
        /*END*/
    )
    /*END*/
ORDER BY
