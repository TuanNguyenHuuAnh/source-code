...
as (
	...
	left join JCA_MENU_PATH menu_path
		on menu.ID = menu_path.DESCENDANT_ID
	where
		CODE in /*includeModuleCode*/('ASSET_RETURN') -- 'ROOT'
...
WHERE
    ...
    AND menu.menu_type IN /*menuTypes*/()
    /*END*/
    AND main.depth = 1
	...
	/*IF excludeModuleCode != null && excludeModuleCode.size() > 0*/
		AND menu.CODE not in /*excludeModuleCode*/('')
	/*END*/
ORDER BY
    main.ancestor_id,
    menu.display_order