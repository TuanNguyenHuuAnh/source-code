SELECT 
  ...
  ,f_out.SUBMITTED_DATE			AS SUBMITTED_DATE
...
ON
  acc_assigned.ID = f_out.COMPLETED_ID
LEFT JOIN 
...
ON
  acc_submited.ID = f_out.SUBMITTED_ID
LEFT JOIN 
...
WHERE 
	doc.DELETED_ID = 0 
	AND f_out.COMPLETED_ID = /*accountId*/1
	/*IF efoDocumentFilterSearchDto.companyId !=null*/
  	...
  	/*IF efoDocumentFilterSearchDto.fromDate !=null*/
  		AND f_out.SUBMITTED_DATE >=/*efoDocumentFilterSearchDto.fromDate*/''
  	/*END*/
  	/*IF efoDocumentFilterSearchDto.toDate !=null*/
  		AND f_out.SUBMITTED_DATE <=/*efoDocumentFilterSearchDto.toDate*/''
  	/*END*/
