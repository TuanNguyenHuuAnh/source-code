 ...
 * Author       phunghn
 * Change log   2017/04/2601-00 phunghn create a new
 *****************************************************************************/
...
public class ImportExcelUtil<T> {
	 ...
	 * @since 13-12-2017
	 * @author PhucNV
	 * @description Refactoring & Fix error when parse boolean to number
	 */
				...
				for (ItemColsExcelDto col : cols) {
					...
					if (currentCell != null && currentCell.getCellTypeEnum() != CellType.BLANK) {
						...
						case INTEGER:
							/**
							 * @author PhucNV
							 * @update !fix error convert from boolean to number
							 * @date 13-12-2017
							 */
	...
	public static <E extends Enum<E>> void setListColumnExcel(Class<E> enumType, List<ItemColsExcelDto> cols) {
		...
		for (E en : enumType.getEnumConstants()) {
			...
			itemCol.setColName(en.name().toUpperCase());
			itemCol.setColIndex(Integer.parseInt(en.toString().trim()));
			cols.add(itemCol);
		}
	}
	
	public static <E extends Enum<E>> void setListColumnExcelWithEnumIndex(Class<E> enumType, List<ItemColsExcelDto> cols) {
		// loop enum
		for (E en : enumType.getEnumConstants()) {
			ItemColsExcelDto itemCol = new ItemColsExcelDto();
			itemCol.setColName(en.name().toUpperCase());
			itemCol.setColIndex(en.ordinal());
			cols.add(itemCol);
		}
	}
}
