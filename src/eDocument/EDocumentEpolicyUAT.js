...
import EDocumentDetail from "../SDK/EDocumentDetail";
...
class EDocumentEpolicy extends Component {
            ...
            .then(Res => {
                ...
                if (Response.Result === 'true') {
                    if (Response.ClientProfile) {
                        let profile = Response.ClientProfile;//this.sortListEffectDate(this.sortListByStatus(Response.ClientProfile));
                        let list = null;
    ...
    handleLoadMore = () => {
        ...
        if (this.state.lastDisplayedIndex === this.state.clientProfile?.length) {
            ...
            this.cpGetEPolicys(this.state.Offset + 1);
        } else {
    ...
    render() {
        ...
        const showCardInfo = (polID, index) => {
            ...
            //     deviceId: getDeviceId()
            // };
            let test = "{\"sourceSystem\":\"DSuccess\",\"group\":\"GROUP_AD_AGENT_DLVN\",\"policyNo\":\"003904872\",\"proposalNo\":\"\",\"deviceId\":\"37d81149f11e4538a7b879760c09e2a9\"}";
            let escryptTest = AES256.encrypt(test, COMPANY_KEY3);
            console.log("testtttttt=", escryptTest);
            // jsonState.data = escryptTest;
            this.setState(jsonState);
