...
import {API_BASE_URL, AUTHENTICATION, COMPANY_KEY, FE_BASE_URL, WEB_BROWSER_VERSION} from '../../../../../constants';
...
const PasscodeCheck = () => {
    ...
    const [isOpenND13, setIsOpenND13] = useState(false);
    ...
    const checkLinkExpire = () => {
        const request = {
            jsonDataInput: {
                ...
                Project: "mcp",
                LinkPath: FE_BASE_URL + '/13/' + id,
            }
        };

        CPConsentConfirmation(request).then(res => {
            let Response = res.Response;
            if ((Response.ErrLog === 'Link_Expired') || (Response.ErrLog === 'Link_Completed')) {
                ...
                setPassRemain('');
                setLoading(false);
            } else if (Response.ErrLog === 'PassCode_Lock') {
    ...
    const popupLaPassSubmit = (event) => {
        ...
        CPConsentConfirmation(request).then(res => {
            ...
            } else if ((res.Response.ErrLog === 'Link_Expired')  || (res.Response.ErrLog === 'Link_Completed')) {
                setIsNotAvailable(false);
                setIsExpired(true);
                setIsOpenPassCode(false);
                setIsOpenND13(false);
    ...
    return (<div className='cus-popup'>
        ...
        {isExpired && !isOpenPassCode && (<ND13LILinkExpired />)}

        {isOpenND13 && <ND13LIConfirm
            clientProfile={clientProfile}
            newAPIToken={newAPIToken}
            userLogin={userLogin}
        />}

        {!isExpired && isOpenPassCode && <div className="popup special show">
            <form onSubmit={(event)=>popupLaPassSubmit(event)}>
