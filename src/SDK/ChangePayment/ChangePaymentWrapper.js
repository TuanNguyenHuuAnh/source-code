...
import PaymentPolList from './PaymentPolList';
import ChangePaymentDetail from './ChangePaymentDetail';
...
class ChangePaymentWrapper extends Component {
    constructor(props) {
        super(props);
        ...
        this.state = {
            ...
            selectedItem: '',
            isConfirm: false,
            subStepName: PAYMENT_SUB_STATE.INIT,
    ...
    componentDidUpdate() {
        if (this.props.proccessType !== this.state.proccessType) {
            this.setState({proccessType: this.props.proccessType, polID: '', selectedItem: null, stepName: FUND_STATE.CHOOSE_POLICY});
        }
    }
    ...
    render() {
        const handlerBackToPrevStep = () => {
            if (this.state.stepName === FUND_STATE.UPDATE_INFO) {
                if (this.state.subStepName > PAYMENT_SUB_STATE.INIT) {
                ...
                if ((this.state.stepName === FUND_STATE.CHOOSE_POLICY) && (this.props.appType !== 'CLOSE')) {
                    ...
                    let obj = {
                        Action: "BACK_ND13_" + this.props.proccessType,
        ...
        const showCardInfo = (item) => {
            const jsonState = this.state;
            jsonState.polID = item.PolicyID;
            jsonState.selectedItem = item;
        ...
        const ChangePayment = () => {
            ...
            onlineRequestSubmit(jsonRequest).then(Res => {
                ...
                if (Response.Result === 'true' && Response.ErrLog === "EXIST") {
                    ...
                    if ((this.props.proccessType === 'Partial Withdrawal') && (this.state?.selectedItem?.PolicyClassCD === 'IL')) {
                        getFundList(this.state.polID);//Partial Withdrawal IL
                    } else {
            ...
            CPGetPolicyInfoByPOLID(apiRequest).then(Res => {
                ...
                if (Response.ErrLog === 'SUCCESSFUL') {
                    if (Response.ClientProfile) {
                        ...
                        console.log('fund list=', Response.ClientProfile);
                    }
        ...
        if (this.props.noValidPolicy) {
            msg = 'Chúng tôi không tìm thấy hợp đồng đủ điều kiện thực hiện yêu cầu ' + SUBMISSION_TYPE_MAPPING[this.props.proccessType] + '. Quý khách vui lòng kiểm tra lại.';
        ...
        return (
            <>
                ...
                {(this.props.noValidPolicy) ? (
                    <div className="sccontract-container" style={{backgroundColor: '#ffffff'}}>
                        <div className="insurance">
                            <div className="empty">
                                <div className={this.props.noValidPolicy ? "picture" : "icon"}
                ...
                <div className="sccontract-container" style={{backgroundColor: '#f5f3f2'}}>
                    ...
                    {this.state.stepName === FUND_STATE.CHOOSE_POLICY ? (
                        this.state.deviceId && this.props.proccessType &&
                        <PaymentPolList 
                        polListProduct={this.state.polListProduct} 
                        polID={this.state.polID} 
                        clientId={this.state.clientId}
                        proccessType={this.props.proccessType}
                        acb={this.state.polListProduct}
                        deviceId={this.state.deviceId}
                        apiToken={this.state.apiToken}
                        ...
                        <div className='paymode-margin-bottom' style={{display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
                            <div className="bottom-text"
                                ...
                                <p style={{textAlign: 'justify'}}>
                                    ...
                                    style={{color: '#727272'}}>
                                    <p style={{textAlign: 'left'}}>- Số tiền có thể tạm ứng đang hiển thị là giá trị tạm tính và có thể thay đổi vào ngày chúng tôi thực hiện yêu cầu của Quý khách. Trong trường hợp số tiền yêu cầu tạm ứng không thỏa, chúng tôi sẽ thông báo đến Quý khách.</p>
                                    <p style={{textAlign: 'left'}}>- Yêu cầu sẽ phát sinh Khoản giảm thu nhập đầu tư/Phí tạm ứng.</p>
                                    </span>
                                </p>
                            </div>
                        </div>
                    ...
                    {/* {this.state.appType && this.state.trackingId && this.state.polID && (this.state.stepName === FUND_STATE.SDK) && <ND13
                                appType={this.state.appType}
                                trackingId={this.state.trackingId}
                                ...
                                polID={this.state.polID}
                    />} */}
                     {/* {this.state.appType && (this.state.appType !== 'CLOSE') && this.state.trackingId && this.state.polID && this.state.apiToken && this.state.clientName && (this.state.stepName === FUND_STATE.SDK) && 
                    <ND13
                                appType={this.state.appType}
                                trackingId={this.state.trackingId}
                                clientListStr={this.props.clientListStr?this.props.clientListStr:getSession(CLIENT_ID)}
...
export default ChangePaymentWrapper;