...
import {formatFullName, getDeviceId, getSession, showMessage, formatDate, getUrlParameter, trackingEvent, cpSaveLogSDK} from '../../util/common';
import {CPServicesPSPaymentProcessList, logoutSession} from '../../util/APIUtils';
...
class PaymentPolList extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    getPolicyListProduct() {
        const apiRequest = {
            jsonDataInput: {
                Company: COMPANY_KEY,
                Action: 'PSPaymentPolList',
    ...
    render() {
        const choosePolicy = (item) => {
            this.props.showCardInfo(item);
        }
            ...
            <>
                ...
                <section className={getSession(IS_MOBILE)?'section-container-mobile':'section-container'}>
                    <div className="personform" style={{height: 'auto', overflowY: 'visible', paddingBottom: '48px'}}>
                        <h5 className="basic-bold basic-text-upper">chọn hợp đồng</h5>
                        <div className="card-warpper" style={{marginBottom: '-22px', marginTop: '16px'}}>
                            {this.state.polListProduct && this.state.polListProduct.map((item, index) => (
                                    ...
                                    <div
                                        ...
                                        <div className="card__footer">
                                            <div className="card__footer-item">
                                                <p>Ngày hiệu lực</p>
                                                <p className="basic-red basic-semibold">{item.PolIssEffDate?formatDate(item.PolIssEffDate): '-'}</p>
                                            </div>
                                            <div className="card__footer-item">
                    ...
                    {getSession(IS_MOBILE)&&
                        <div className='nd13-padding-bottom96'></div>
                    }
                    {!getSession(IS_MOBILE) &&
