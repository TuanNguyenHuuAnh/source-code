...
import { ACCESS_TOKEN, CLIENT_ID, USER_LOGIN, AUTHENTICATION, COMPANY_KEY, CLAIM_TYPE, FE_BASE_URL, CLAIM_STEPCODE, IS_MOBILE, PageScreen, SIGNATURE } from '../../../../sdkConstant.js';
import { getDocType } from '../../../../sdkAPI.js';
import { getSession, checkListRequireContain, getDeviceId, getBenifits, cpSaveLog, getUrlParameter } from '../../../../sdkCommon.js';
...
import AlertPopupError from '../../../../components/AlertPopupError.js';
    ...
    this.state = {
      jsonInput: {
        jsonDataInput: {
          ...
          APIToken: this.props.apiToken || getSession(ACCESS_TOKEN),
          Project: 'mcp',
          UserLogin: getSession(USER_LOGIN),
          ClientID: getSession(SIGNATURE) || getSession(CLIENT_ID),
          Action: 'DocTypeGroup',
        }
      },
    ...
    const processFile = (file) => {
      return new Promise(async (resolve, reject) => {
        ...
        if (validateImage.includes(file.type)) {
          try {
            let options = {
              maxSizeMB: 0.2, // Giới hạn kích thước file sau khi nén
            ...
            if (fileSize > 0.1) {
              options = {
                maxSizeMB: 0.1, // Giới hạn kích thước file sau khi nén
                maxWidthOrHeight: 1920, // Giới hạn kích thước chiều rộng hoặc chiều cao
                useWebWorker: true, // Sử dụng Web Workers để cải thiện hiệu suất
                initialQuality: 0.2
              };
      ...
      <section className="sccontract-warpper" id="scrollAnchor">
        <div className="insurance">
                ...
                <div className="step-wrapper">
                  <div className="step-btn-wrapper">
                    <div className="back-btn">
                      <button>
                        <div className="svg-wrapper"  onClick={() => this.props.handlerBackToPrevStep(this.props.handlerGetStep(CLAIM_STEPCODE.ATTACHMENT))}>
                          <svg
                            width="11"
                            height="15"
                            viewBox="0 0 11 15"
                          ...
                          {!getSession(IS_MOBILE) ? (
                            <span className="simple-brown" onClick={() => this.props.handlerBackToPrevStep(this.props.handlerGetStep(CLAIM_STEPCODE.ATTACHMENT))}>Quay lại</span>
                          ) : (
              ...
              {attachmentData.map((docTypeData, index) => (
                <div className={index !== 0 ? "info padding-top32" : "info  padding-top12"} key={index}>
                  ...
                  <div className="info__title margin-bottom0">
                    ...
                    <i className="invoke-hoso" onClick={() => this.handlerOpenPopup(docTypeData)} ><img src={FE_BASE_URL + "/img/icon/step/step_3_info.svg"} alt="" /></i>
                  </div>
          ...
          <div className="bottom-btn">
            <button className={(jsonState.disabledButton || this.props.isSubmitting) ? "btn btn-primary disabled" : "btn btn-primary"}
              id="submitClaimDetail" disabled={!!jsonState.disabledButton || this.props.isSubmitting}
              onClick={() => validAndSubmit()}>{this.props.pinStep?'Lưu thông tin': 'Tiếp tục'}</button>
          </div>
        </div>
