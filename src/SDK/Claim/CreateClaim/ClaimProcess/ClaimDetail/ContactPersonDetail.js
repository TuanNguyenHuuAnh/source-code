...
import { getSession, VALID_EMAIL_REGEX, removeAccents, onlyLettersAndSpaces, is18Plus, getBenifits, checkValue, isHC3, cpSaveLog, getUrlParameter } from "../../../../sdkCommon.js";
import { CLAIM_STATE, PAYMENT_METHOD_CASE } from "../../CreateClaimSDK";
import dayjs from 'dayjs';
import { ADDRESS, CELL_PHONE, CLAIM_TYPE, EMAIL, FULL_NAME, POID, DOB, FE_BASE_URL , CLAIM_STEPCODE, IS_MOBILE, PageScreen, SDK_REQUEST_STATUS } from "../../../../sdkConstant.js";
import WarningPopupChange from "../../../../components/WarningPopupChange.js";
...
class ContactPersonDetail extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.claimCheckedMap[CLAIM_TYPE.DEATH]) {
    ...
    } else if ((!is18Plus(this.props.selectedCliInfo.dOB) || this.props.claimCheckedMap[CLAIM_TYPE.ACCIDENT] || this.props.claimCheckedMap[CLAIM_TYPE.ILLNESS] || this.props.claimCheckedMap[CLAIM_TYPE.TPD]|| isHC3(this.props.claimCheckedMap, this.props.claimTypeList)) && (this.props.paymentMethodState.choseReceiver !== 'LI')) {
      if (getSession(FULL_NAME) || this.props.fullName) {
        contactState.contactPersonInfo.fullName = getSession(FULL_NAME) || this.props.fullName;
      }
      
      if (!contactState.contactPersonInfo.pin && (getSession(POID) || this.props.idNum)) {
        contactState.contactPersonInfo.pin = getSession(POID) || this.props.idNum;
      }
      if (!contactState.contactPersonInfo.dob && getSession(DOB)) {
      ...
      if (!contactState.contactPersonInfo.email) {
        contactState.contactPersonInfo.email = this.props.selectedCliInfo.email.Email;
      } 
      if (!contactState.contactPersonInfo.address) {
      ...
      if (this.props.paymentMethodState.paymentMethodCase === PAYMENT_METHOD_CASE.FULL_ABOVE_18) {
        if (!contactState.contactPersonInfo.role || contactState.contactPersonInfo.role === 'PO') {
          ...
          contactState.contactPersonInfo.dob = getSession(DOB);
          contactState.contactPersonInfo.phone = checkValue(getSession(CELL_PHONE));
          contactState.contactPersonInfo.email = checkValue(getSession(EMAIL));
    ...
    //Fix HC3 loose default fullName
    if (!contactState.contactPersonInfo.fullName) {
      if (!this.props.paymentMethodState.choseReceiver) {
        if (!contactState.contactPersonInfo.fullName && getSession(FULL_NAME)) {
          contactState.contactPersonInfo.fullName = getSession(FULL_NAME);
        }
        if (!contactState.contactPersonInfo.pin) {
          contactState.contactPersonInfo.pin = getSession(POID) ? getSession(POID) : '';
  ...
  updateState(contactState) {
    ...
    let isInvalidValue = contactState.contactPersonInfo.errors.dob || contactState.contactPersonInfo.errors.email;
    contactState.disabledButton = isInvalid || isInvalidValue;
  ...
  render() {
    ...
    const changeSelectReceiver = (e) => {
      ...
      if (e.target.value === 'LI') {
        ...
        contactState.contactPersonInfo.pin = this.props.selectedCliInfo.idNum;
        contactState.contactPersonInfo.dob = this.props.selectedCliInfo.dOB;
        contactState.contactPersonInfo.phone = checkValue(this.props.selectedCliInfo.cellPhone);
        contactState.contactPersonInfo.email = checkValue(this.props.selectedCliInfo.email);
        contactState.contactPersonInfo.address = this.props.selectedCliInfo.address;
      ...
      <section className="sccontract-warpper" id="scrollAnchor">
        <div className="insurance">
              ...
              <div className="step-container">
                <div className="step-wrapper">
                  <div className="step-btn-wrapper">
                    <div className="back-btn" onClick={() => this.props.handlerBackToPrevStep(this.props.currentState)}>
                      <button>
                        ...
                        {/* <span className="simple-brown" onClick={() => this.props.handlerBackToPrevStep(this.props.handlerGetStep(CLAIM_STEPCODE.CONTACT))} >Quay lại</span> */}
                        {!getSession(IS_MOBILE) ? (
                            <span className="simple-brown" onClick={() => this.props.handlerBackToPrevStep(this.props.handlerGetStep(CLAIM_STEPCODE.CONTACT))}>Quay lại</span>
                        ):(
                    ...
                    <div className="step-btn-save-quit">
                      <div className='save-quit-wrapper' onClick={this.props.handleSaveLocalAndQuit}>
                        <button>
                  ...
                  <div className="progress-bar">
                    <div className={(this.props.currentState >= this.props.handlerGetStep(CLAIM_STEPCODE.CLAIM_TYPE)) ? "step active" : "step"}>
                      <div className="bullet">
                        <span>1</span>
                      </div>
                      <p>Thông tin sự kiện</p>
                    </div>
                    <div className={(this.props.currentState >= this.props.handlerGetStep(CLAIM_STEPCODE.PAYMENT_METHOD)) ? "step active" : "step"}>
                      <div className="bullet">
                        <span>2</span>
                      </div>
                      <p>Thanh toán và liên hệ</p>
                    </div>
                    <div className={(this.props.currentState >= this.props.handlerGetStep(CLAIM_STEPCODE.ATTACHMENT)) ? "step active" : "step"}>
                      <div className="bullet">
                        <span>3</span>
                      </div>
                      <p>Kèm <br />chứng từ</p>
                    </div>
                    <div className={(this.props.currentState >= this.props.handlerGetStep(CLAIM_STEPCODE.REVIEW)) ? "step active" : "step"}>
                      <div className="bullet">
                        <span>4</span>
                      </div>
                        ...
                        <div className="tab">
                          <div className="input">
                            <div className="input__content">
                              ...
                              <NumberFormat displayType="input" type="tel" maxLength="12" value={this.props.contactPersonInfo.phone} prefix=""
                            ...
                            <i><img src={FE_BASE_URL + "/img/icon/input_phone.svg"} alt="" /></i>
                          </div>
                        </div>
                        {/* Email */}
                        <div className="tab">
                          <div className="input">
                            <div className="input__content">
                              ...
                              <input type="search" placeholder="Email" maxLength="100" value={this.props.contactPersonInfo.email?this.props.contactPersonInfo.email: ''}
                                onChange={this.handlerOnChangeEmail} />
                            </div>
                            <i><img src={FE_BASE_URL + "/img/icon/email.svg"} alt="" /></i>
                          </div>
                        </div>
          ...
          <div className="bottom-btn">
            <button className={(this.props.contactState.disabledButton || this.props.isSubmitting) ? "btn btn-primary disabled" : "btn btn-primary"}
              id="submitContactDetail" disabled={!!this.props.contactState.disabledButton || this.props.isSubmitting}
              onClick={() => checkSubmit()}>{this.props.pinStep?'Lưu thông tin': 'Tiếp tục'}</button>
          </div>
        </div>
        {this.state.isPOInfoDiff &&
          <WarningPopupChange closePopup={closePOInfoDiff} parentSubmit={this.props.handlerSubmitContact} msg={parse("<p>Thông tin <span className='basic-bold'>" + this.state.msg + "</span> của <span className='basic-bold'>" + this.props.contactPersonInfo.fullName + "</span> đang khác với thông tin đã cung cấp trên hợp đồng bảo hiểm, nên thông tin liên hệ chỉ áp dụng cho yêu cầu này.</p><p>Quý khách vui lòng vào tính năng “Điều chỉnh thông tin hợp đồng” hoặc đến văn phòng Dai ichi Life Việt Nam gần nhất để cập nhật thông tin mới.</p>")} imgPath={FE_BASE_URL + "/img/popup/contact-info.svg"} />
        }
      </section>
    );
  }
