...
import {formatFullName, getUrlParameter, getSession, showMessage, trackingEvent, cpSaveLogSDK} from '../../util/common';
import {CPGetPolicyListByCLIID, logoutSession, onlineRequestSubmit} from '../../util/APIUtils';
...
class SARiderPolList extends Component {
    constructor(props) {
        super(props);
        this.state = {
            polListProduct: this.props.polListProduct,
            submitIn24: false,
            PolicyID: '', 
            PolicyLIName: '', 
            Frequency: '', 
                            ...
                            {this.state.polListProduct && this.state.polListProduct.map((item, index) => (
                                <div className="item" style={{padding: '6px 0'}}
                                    ...
                                    <div
                                        ...
                                        onClick={() => choosePolicy(item.PolicyID, item.PolicyLIName, item.Frequency, item.PolMPremAmt, item.IsChangeFrequency, item.PolicyClassCD, item.FrequencyCode, item.PolSndryAmt, item.IsDegrading)}
                                        id={index}>
