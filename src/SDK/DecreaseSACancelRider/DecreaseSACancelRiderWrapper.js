...
import {
    getDeviceId,
    getSession,
    showMessage, trackingEvent, setLocal,
    removeLocal
} from '../../util/common';
import POWarningND13 from "../ModuleND13/ND13Modal/POWarningND13/POWarningND13";
import ND13 from "../ND13";
import SARiderPolList from './SARiderPolList';
import SARiderDetail from './SARiderDetail';
...
class DecreaseSACancelRiderWrapper extends Component {
    constructor(props) {
        super(props);
        ...
        this.handlerPolListProduct = this.updatePolListProduct.bind(this);
        this.handlerUpdateConfirm = this.updateConfirm.bind(this);
    }
    ...
    callAPIChangeSAByPolicyID(PolicyId) {
        ...
        CPGetPolicyInfoByPOLID(apiRequest).then(Res => {
            ...
            if (Response.ErrLog && Response.ErrLog === 'SUCCESSFUL') {
                if (Res.Response.ClientProfile) {
                    ...
                    console.log('Response.ClientProfile=', Response.ClientProfile);
                    // let profileGroupBy = this.trimProfileData(Response.ClientProfile);
                    let profileGroupBy = this.groupbyProfileByLI(Response.ClientProfile);
                    console.log('groupbyLI=',profileGroupBy);
                    profileGroupBy = this.groupbyByProduct(profileGroupBy);
                    console.log('groupbyByProduct=', profileGroupBy);
                    profileGroupBy = this.sumProductByLI(profileGroupBy);
                    console.log('sumProductByLI=', profileGroupBy);
                    jsonState.ProfileGroupBy = profileGroupBy;
                }
    ...
    sumProductByLI(products) {
        console.log('products.length=', products.length);
        for (let t = 0; t < products.length; t++) {
            console.log('products[t].Products.length=', products[t].Products.length);
            for (let u = 0; u < products[t].Products.length; u++) {
                console.log('products[t].Products[u].Profiles?.length=', products[t].Products[u].Profiles?.length);
                if ((products[t].Products[u].Profiles?.length > 1) && (products[t].Products[u].PolicyGroup !== '3')) {
                    let item = products[t].Products[u].Profiles[0];
                    console.log('products[t].Products[u].Profiles.length=', products[t].Products[u].Profiles.length);
                    for (let i = 1; i < products[t].Products[u].Profiles.length; i++) {
                        if (products[t].Products[u].Profiles[i].LifeInsureID === item.LifeInsureID) {
                            let PolMPremAmt0 = (item.PolMPremAmt?item.PolMPremAmt: '0').replaceAll('.', '');
                            let PolMPremAmt = (products[t].Products[u].Profiles[i].PolMPremAmt?products[t].Products[u].Profiles[i].PolMPremAmt: '0').replaceAll('.', '');
                            let sumMPremAmt = (parseInt(PolMPremAmt0) + parseInt(PolMPremAmt)).toLocaleString('de-DE');
    
                            let FaceAmount0 = (item.FaceAmount?item.FaceAmount: '0').replaceAll('.', '');
                            let FaceAmount = (products[t].Products[u].Profiles[i].FaceAmount?products[t].Products[u].Profiles[i].FaceAmount: '0').replaceAll('.', '');
                            let sumFaceAmount = (parseInt(FaceAmount0) + parseInt(FaceAmount)).toLocaleString('de-DE');
                            console.log('sumMPremAmt=', sumMPremAmt);
                            console.log('sumFaceAmount=', sumFaceAmount);
                            item.PolMPremAmt = sumMPremAmt;
                            item.FaceAmount = sumFaceAmount;
                            products[t].Products[u].Profiles[i] = item;
                            products[t].Products[u].Profiles.splice(0, 1);
                            i--;
                            // console.log('product=', products[t].Profiles[i]);
                        }
                    }
                } else if ((products[t].Products[u].Profiles?.length > 1) && (products[t].Products[u].PolicyGroup === '3')) {
                    let profiles = products[t].Products[u].Profiles;
                    ...
                    // let order = ["ULHI41", "ULHO41", "ULHD41"];
                    // profiles.sort((a, b) => order.indexOf(a.ProductCode.trim()) - order.indexOf(b.ProductCode.trim()));
                    products[t].Products[u].Profiles = profiles;
                }
            }
    ...
    groupbyByProduct = (profiles) => {
        const groupedProducts = profiles.reduce((product, profile) => {
            const { ProductName, IsMainCvg, PolicyGroup } = profile;
            if (!product[ProductName + '@' + IsMainCvg + '@' + PolicyGroup]) {
                product[ProductName + '@' + IsMainCvg + '@' + PolicyGroup] = [];
            }
            product[ProductName + '@' + IsMainCvg + '@' + PolicyGroup].push(profile);
          ...
          const result = Object.keys(groupedProducts).map(key => (
            key && (key.split('@').length === 3) &&
            {
                ...
                IsMainCvg: key.split('@')[1],
                PolicyGroup: key.split('@')[2],
    ...
    // groupbyProfileByLI = (profiles) => {
    //     const groupedProfiles = profiles.reduce((li, profile) => {
    //         const { LifeInsureID, PolicyGroup, IsMainCvg, PlanCode } = profile;
    //         if (PolicyGroup === '3') {
    //             if (!li[LifeInsureID + '-' + PolicyGroup + '-' + IsMainCvg]) {
    //                 li[LifeInsureID + '-' + PolicyGroup + '-' + IsMainCvg] = [];
    //             } 
    //             li[LifeInsureID + '-' + PolicyGroup + '-' + IsMainCvg].push({...profile});
    //         } else {
    //             if (!li[LifeInsureID + '-' + PolicyGroup + '-' + IsMainCvg + '-' + PlanCode]) {
    //                 console.log(LifeInsureID + '-' + PolicyGroup + '-' + IsMainCvg);
    //                 li[LifeInsureID + '-' + PolicyGroup + '-' + IsMainCvg + '-' + PlanCode] = [];
    //             }
    //             li[LifeInsureID + '-' + PolicyGroup + '-' + IsMainCvg + '-' + PlanCode].push({...profile});
    //         }
  
    //         return li;
    //       }, {});
          
    //       const result = Object.keys(groupedProfiles).map(key => (
    //         key && (key.split('-').length >= 3) &&
    //         {
    //             LifeInsureID: key.split('-')[0],
    //             PolicyGroup: key.split('-')[1],
    //             IsMainCvg: key.split('-')[2],
    //             ProductName: groupedProfiles[key][0]?.ProductName.split('-')[0],
    ...
    // Function to trim spaces in ProductCode and PlanCode
    trimProfileData = (profiles) => {
        return profiles.map(profile => ({
            ...profile,
            ProductCode: profile.ProductCode.trim(),
            PlanCode: profile.PlanCode.trim()
        }));
    };
    ...
    groupbyProfileByLI = (profiles) => {
        const groupedProfiles = profiles.reduce((li, profile) => {
            ...
            let key;
            if (PolicyGroup === '3') {
                key = `${LifeInsureID}-${PolicyGroup}-${IsMainCvg}`;
            } else {
                key = `${LifeInsureID}-${PolicyGroup}-${IsMainCvg}-${PlanCode}`;
            }
    ...
    setSubStepName = (subStepName) => {
        this.setState({subStepName: subStepName});
    }

    
    updatePolListProduct = (profile) => {
        this.setState({polListProduct: profile});
    }
    ...
    render() {
        ...
        const handlerBackToPrevStep = () => {
            if (this.state.stepName === FUND_STATE.UPDATE_INFO) {
                if (this.state.subStepName > SUB_STATE.INIT) {
                    this.setState({subStepName: this.state.subStepName - 1});
                } else {
        ...
        const showCardInfo = (polID, PolicyLIName, Frequency, PolMPremAmt, IsChangeFrequency, PolicyClassCD, FrequencyCode, PolSndryAmt, IsDegrading, forceContinue) => {
            ...
            // if (forceContinue) {
            //     this.callAPIChangeSAByPolicyID(polID);
            // }
        ...
        const ChangeSA = () => {
            ...
            onlineRequestSubmit(jsonRequest).then(Res => {
                ...
                if (Response.Result === 'true' && Response.ErrLog === "EXIST") {
                    ...
                    this.callAPIChangeSAByPolicyID(this.state.polID);
                }
            ...
            <>
                ...
                <div className="sccontract-container" style={{backgroundColor: '#f5f3f2'}}>
                    ...
                    {this.state.stepName === FUND_STATE.CHOOSE_POLICY ? (
                        this.state.deviceId && 
                        <SARiderPolList 
                        polListProduct={this.state.polListProduct} 
                        ...
                        handlerPolListProduct={this.handlerPolListProduct}
                        ...
                        ((this.state.stepName === FUND_STATE.UPDATE_INFO) || (this.state.stepName === FUND_STATE.VERIFICATION) || (this.state.stepName === FUND_STATE.SDK)) && (
                            ...
                            <SARiderDetail 
                                ...
                                subStepName={this.state.subStepName}
                                PlanCodeHCMap={this.state.PlanCodeHCMap}
                                handlerUpdateConfirm={this.handlerUpdateConfirm}
                        ...
                        <div className="bottom-btn" style={{maxWidth: '578px', paddingBottom: '36px'}}>
                                {this.state.polID ? (
                                    <button className="btn btn-primary" onClick={() => {
                                        this.cpSaveLog(`Web_Open_${PageScreen.POL_TRANS_CHANGE_DECREASE_SA}`);
                                        trackingEvent(
                                            "Giao dịch hợp đồng",
                                            `Web_Open_${PageScreen.POL_TRANS_CHANGE_DECREASE_SA}`,
                                            `Web_Open_${PageScreen.POL_TRANS_CHANGE_DECREASE_SA}`,
                                        );
                                        // getFundList();
                                        ChangeSA();
                ...
                <ErrorReInstamentSubmitted closePopup={() => closeSubmitIn24()} msg={'Hợp đồng này đã có yêu cầu thay đổi số tiền bảo hiểm/chấm dứt sản phẩm bổ trợ. Quý khách có muốn tiếp tục tạo yêu cầu không?'} imgPath={'../../img/popup/submited-in-24.svg'}
                                            screen={''}
...
export default DecreaseSACancelRiderWrapper;