...
import { CPSaveLog, getEPolicyPdf, getEdocument, getOmiDocs, getRecordingToken, getDocTypePermission, getIBPSDocuments } from './sdkAPI';
...
const EDocumentDetailWrapper = (props) => {
    ...
    const getOmiDocuments = (jsonData, encryptedText, remainRetryTimes) => {
                ...
                try {
                    getIBPSDocuments(jsonData, encryptedText).then(Response => {
                        // let Response = Res.Response;
                    ...
                    !state.showRuleDetail ? (<div className="an-hung-page">
                        ...
                        <div className="bo-hop-dong-container">
                            ...
                            {state.omiDocProfile && state.omiDocProfile.map((item, index) => {
                                let permission = getPermisson(item, state.docTypePermission);
                                return (
                                    permission&&
                                    <>
                                    <div className={!props.data?"bo-hop-dong-container-body":"bo-hop-dong-container-body-sdk"} key={'omi' + index}
                                            style={{cursor: 'pointer'}}
                                            onClick={() => openOmiDoc(item.content, item.mimeType, 1, permission)}>
...
export default EDocumentDetailWrapper;