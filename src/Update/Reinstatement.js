...
class Reinstatement extends Component {
    ...
    popupOtpSubmit = (OTP) => {
        // submit reinstament
        this.submitReInstament(OTP);
    }

    submitReInstament = (OTP) => {
        let list = [];
        ...
        onlineRequestSubmit(submitRequest)
            .then(res => {
                if ((res.Response.Result === 'true') && (res.Response.ErrLog === 'Submit RIN is saved successfull.') && res.Response.Message) {
                    ...
                    // upload images
                    this.uploadImages(res.Response.Message);
    ...
    showCardInfoLapse = (policyID, CountDayLapse, YearLapse, PolExpiryDate, item) => {
        const jsonState = this.state;
        jsonState.polID = policyID;
        ...
        this.checkSubmitedIn24h(policyID);
    }
        ...
        if (!getSession(POL_LIST_LAPSE)) {
            ...
            CPGetPolicyListByCLIID(submitRequest).then(Res => {
                ...
                if (Response.ErrLog === 'SUCCESSFUL' && Response.ClientProfile && Response.ClientProfile.length > 0) {
                    ...
                    jsonState.stepName = FUND_STATE.CHOOSE_POLICY;
                    if (this.props.match.params.info && this.props.match.params.info.indexOf('-') < 0) {
                        const itemLapse =  Response.ClientProfile.find(item => item.PolicyID === this.props.match.params.info);
                        jsonState.proccessType = 'RIN';
                        jsonState.polID = itemLapse.PolicyID;
                        this.setState(jsonState);
                        this.showCardInfoLapse(itemLapse.PolicyID, itemLapse.CountDayLapse, itemLapse.YearLapse, itemLapse.PolExpiryDate, itemLapse);
                    } else {
                        this.setState(jsonState);
                    }
            ...
            if (this.props.match.params.info && this.props.match.params.info.indexOf('-') < 0) {
                const itemLapse =  polListLapse.find(item => item.PolicyID === this.props.match.params.info);
                jsonState.proccessType = 'RIN';
                jsonState.polID = itemLapse.PolicyID;
                this.setState(jsonState);
                this.showCardInfoLapse(itemLapse.PolicyID, itemLapse.CountDayLapse, itemLapse.YearLapse, itemLapse.PolExpiryDate, itemLapse);
    ...
    checkSubmitedIn24h = () => {
        ...
        onlineRequestSubmit(jsonRequest).then(Res => {
            ...
            if (Response.Result === 'true' && Response.ErrLog === "EXIST") {
                ...
                setSession(SUBMIT_IN_24_RIN, SUBMIT_IN_24_RIN);
                return;
            }
        }).catch(error => {
        });
    ...
    render() {
        ...
        const goToChoosePolicy = () => {
            ...
            if (jsonState.loading) {
                setTimeout(goToChoosePolicy, 1000);
                return;
            }
        ...
        const goToChooseLapsePolicy = () => {
            let jsonState = this.state;
            if (jsonState.loading) {
                setTimeout(goToChooseLapsePolicy, 1000);
                return;
            }
        ...
        const EnterInfoToReinstament = () => {
            ...
            if (isInteger(this.state.polLapseSelected?.YearLapse) && (parseInt(this.state.polLapseSelected?.YearLapse) >= 1)) {
            ...
            onlineRequestSubmit(submitRequest)
                .then(res => {
                    if ((res.Response.Result === 'true') && (res.Response.ErrLog === 'Successfull.') && res.Response.ClientProfile) {
                        this.setState({insuredLapseList: res.Response.ClientProfile});
                    } else {
                        // alert('no data');
                        this.setState({insuredLapseList: null});
                    }
                }).catch(error => {
                // alert(error);
            });
        ...
        const hideUpload = () => {
            this.setState({uploadSelected: false});
        }
        ...
        const checkSubmitedIn24h = (polID) => {
            if (getSession(SUBMIT_IN_24_RIN + polID)) {
                return;
            }
            const jsonRequest = {
                jsonDataInput: {
            ...
            onlineRequestSubmit(jsonRequest).then(Res => {
                ...
                if (Response.Result === 'true' && Response.ErrLog === "EXIST") {
                    // this.setState({submitIn24: true});
                    setSession(SUBMIT_IN_24_RIN + polID, SUBMIT_IN_24_RIN);
                    return;
                }
            }).catch(error => {
            });
        ...
        console.log('render passAnswer=', passAnswer);
        let msg = '';
        let imgPath = '';
        let msgPopup = '';
        let popupImgPath = '';
        ...
        if (this.state.apiError) {
            msgPopup = 'Có ảnh đính kèm không thành công, vui lòng thử lại';
        ...
        if (this.state.submitIn24) {
            msgPopup = 'Hợp đồng này đã có yêu cầu khôi phục hợp đồng. Quý khách có muốn tiếp tục tạo yêu cầu không?';
                    ...
                    <div className="main-warpper basic-mainflex">
                        <section className="qlbh-sccard-wrapper sccard-warpper hopdong-sccard-wrapper"
                            ...
                            <div className="stepform stepform-42" style={{marginTop: '0px'}}>
                                <div className="stepform__body" style={{paddingTop: '4px'}}>
                                    <div className="info">
                        ...
                        <section className="sccontract-warpper additional-information-sccontract-warpper">
                            ...
                            <div className="sccontract-container" style={{backgroundColor: '#f5f3f2'}}>
                                    ...
                                    this.state.updateFund ? (
                                                                        ...
                                                                        <div className="dropdown__content"
                                                                            ...
                                                                            {this.state.currentFundsExpandMap[fundId] === false ?
                                                                                <p className="feeDetailsArrow" style={{
                                                                                    ...
                                                                                    src={FE_BASE_URL + "/img/icon/arrow-left-grey.svg"}
                                                                                    alt="arrow-left-icon"
                                                                                    style={{right: '0px'}}/></p> :
                                                                                <p className="feeDetailsArrowDown"
                                                                                   style={{
                                                                                    ...
                                                                                    src={FE_BASE_URL + "/img/icon/arrow-down-grey.svg"}
                                                                                    alt="arrow-down-icon"
                                                                                    style={{right: '0px'}}/></p>}
                                                                        </div>
                                                                        <div
                                        ...
                                        this.state.updateInvestRate ? (
                                                ...
                                                <div className='stepform' style={{marginTop: '175px'}}>
                                                    <div className="an-hung-page" style={{padding: '0 10px'}}>
                                                        <div className="bo-hop-dong-container">
                                                                                                ...
                                                                                                <div
                                                                                                    ...
                                                                                                    <div
                                                                                                        ...
                                                                                                        {this.state.currentFundsChangeMap[fundId].percent === '' &&
                                                                                                            <img
                                                                                                                src={FE_BASE_URL + "/img/icon/edit.svg"}
                                                                                                                alt=""/>
                                                                                                        }
                                                                                                    </div>
                                                            ...
                                                            <img className="decor-clip" src={FE_BASE_URL + "/img/mock.svg"} alt=""/>
                                                            <img className="decor-person" src={FE_BASE_URL + "/img/person.png"}
                                                                 alt=""/>
                                            ...
                                            this.state.stepName === FUND_STATE.UPDATE_INFO ? (
                                                                                            ...
                                                                                            <button
                                                                                                ...
                                                                                                <img style={{
                                                                                                    ...
                                                                                                    height: '24px'
                                                                                                     ...
                                                                                                     src={FE_BASE_URL + "/img/icon/zl_plug.svg"}
                                                                                                     alt="circle-plus"/>
                                                                                            </button>
                                                ...
                                                <p className='inline-red'>{this.state.errorUpload}</p>
                                              } */}
                                                                                            ...
                                                                                            <div className="file">
                                                                                                <div
                                                                                                    ...
                                                                                                    <img
                                                                                                        src={att.imgData}
                                                                                                        alt=""
                                                                                                        onClick={() => openBase64(att.imgData)}
                                                                                                    />
                                                                                                </div>
                                                                                                <div
                                                                                                    className="deletebtn"
                                                    ...
                                                    <div className='stepform' style={{marginTop: '175px'}}>
                                                        ...
                                                        <div className="an-hung-page" style={{padding: '0 10px'}}>
                                                            <div className="bo-hop-dong-container page-eleven"
                                                                ...
                                                                <img className="decor-clip" src={FE_BASE_URL + "/img/mock.svg"}
                                                                     alt=""/>
                                                                <img className="decor-person" src={FE_BASE_URL + "/img/person.png"}
                                                                     alt=""/>
                                ...
                                {this.state.stepName === FUND_STATE.CHOOSE_POLICY && !this.state.updatePeriodicFee && 
