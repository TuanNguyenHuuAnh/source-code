...
let allFundsIL1 = ["ILPF1", "ILPF2", "ILPF3", "ILPF6", "ILPF7"];
...
let allFundsIL2 = ["ILPF1", "ILPF8", "ILPF3", "ILPF6", "ILPF7"];
let allFundsMapIL2 = {
    "ILPF1": "Quỹ Tăng trưởng",
    "ILPF8": "Quỹ Cân bằng",
    "ILPF3": "Quỹ Bảo toàn",
...
export const INITIAL_STATE_UPDATE_POLICY_INFO = () => {
    return {
            ...
            isCompany: false, 
            PolicyGroup: 'IL1', // IL1 and IL2
            isClicked: false
        }
    };

class UpdatePolicyInfo extends Component {
                        ...
                        for (let i = 0; i < allFunds.length; i++) {
                            ...
                            if (fundChange) {
                                fundItem = {
                                    ...
                                    fund_val_each_funf: (fundChange.value === undefined || !isNumber(fundChange.value) ) ? '0' : fundChange.value,
                                ...
                                if (indx >= 0) {
                                    ...
                                    if ((fundRemain.percent !== '') && (fundRemain.percent > 0)) {
                                        fundItem = {
                                            ...
                                            fund_id: allFunds[i],
                                            FundNameCode: allFundsMap[allFunds[i]],
                                            fund_val_each_funf: !isNumber(fundRemain.value) ? '0' : fundRemain.value,
    ...
    initRequestFund = () => {
        if (this.state.updateInvestRate) {
            // alert('complete change rate');
            let reqType = 'FAP';
            if (this.state.forThisOnly) {
                reqType = 'FPM';
            }
            this.setState({proccessType: reqType})
            let jsonState = this.state;
            let newItems = [];
            let oldItems = [];
            for (let i = 0; i < allFunds.length; i++) {
                ...
                if (fundChange) {
                    ...
                    fundItem = {
                        ...
                        fund_id: allFunds[i],
                        FundNameCode: allFundsMap[allFunds[i]],
                        fund_val_each_funf: !isNumber(fundChange.value) ? '0' : fundChange.value,
                        NewRate: (fundChange.percent === '') ? 0 : fundChange.percent,
                        OldRate: fundOriginal.percent,
                        Order: '' + (i + 1)
                    }
            ...
            onlineRequestSubmit(submitRequest)
                .then(res => {
                    if (res.Response.Result === 'true' && res.Response.ErrLog === 'Submit OS is saved successfull.') {
                        ...
                        state.trackingId = res.Response.Message;
    ...
    updateNoValidPolicy = (value) => {
        this.setState({noValidPolicy: value, stepName: FUND_STATE.CATEGORY_INFO});
    }
    ...
    render() {
        ...
        const radioInvestRate = (e) => {
            ...
            jsonState.updateSundryAmount = false;
        ...
        const radioSundryAmount = (e) => {
        e.preventDefault();
        this.cpSaveLog(`Web_Open_${PageScreen.POL_TRANS_CHANGE_SUNDRY_AMOUNT}`);
        trackingEvent(
            "Giao dịch hợp đồng",
        ...
        let jsonState = this.state;
        jsonState.updateSundryAmount = true;
        jsonState.updatePeriodicFee = false;
        jsonState.updateInvestRate = false;
        jsonState.updateFund = false;
        jsonState.insuranceFee = false;
        ...
        jsonState.noValidPolicy = false;
        jsonState.appType = 'CLOSE';
        jsonState.proccessType = 'SundryAmountS';
        if (getSession(GENDER) === 'C') {
            //không dành cho khách hàng doanh nghiệp
            jsonState.isCompany = true;
            this.state.stepName = FUND_STATE.CATEGORY_INFO;
        } else {
            jsonState.stepName = FUND_STATE.CHOOSE_POLICY;
        }
        ...
        const showCardInfo = (polID, index, AgentName, PolicyClassCD) => {        
            ...
            let policyGroup = '';
            if(this.state.polListProfile && index >= 0 && index < this.state.polListProfile.length){
                let pol = this.state.polListProfile[index];
                console.log("POLICY NO: " + pol?.PolicyID + ", POLICY GROUP: " + pol?.PolicyGroup);

                if(pol?.PolicyGroup === 'IL2'){
                    allFunds = allFundsIL2;
                    allFundsMap = allFundsMapIL2;
                } else if(pol?.PolicyGroup === 'IL1'){
                    allFunds = allFundsIL1;
                    allFundsMap = allFundsMapIL1;
                } 
                policyGroup = pol?.PolicyGroup;
            }
            const jsonState = this.state;
            jsonState.PolicyGroup = policyGroup;
        ...
        const getFundList = () => {
            ...
            if (jsonState.lastChosenPolID !== jsonState.polID) {
                ...
                if(jsonState.PolicyGroup === "IL1")
                    jsonState.remainFunds = ["ILPF1", "ILPF2", "ILPF3", "ILPF6", "ILPF7"];
                else if(jsonState.PolicyGroup === "IL2")
                    jsonState.remainFunds = ["ILPF1", "ILPF8", "ILPF3", "ILPF6", "ILPF7"];
        ...
        const enterPercent = (fromFund, exId, fundValue, obj) => {
            let jsonState = this.state;
            if(!isNumber(fundValue)) fundValue = 0;
        ...
        const EnterInfoToReinstament = () => {
            ...
            // if (getSession(SUBMIT_IN_24 + this.state.polID)) {
            //     this.setState({submitIn24: true});
            //     return;
            // }
        ...
        const isPassAnwserList = () => {
            ...
            for (let x in this.state.insuredLapseList[0].Questionaire) {
                ...
                if (this.state.AnsweredListMap[item.LifeInsure].length <= item.QuestionList.length) { //(AnswerList trong map có sẵn 1 phần tử rỗng.) Nếu chưa chọn hết tất cả câu trả lời thì
                    return false;
                }
            }
            return true;
        ...
        if (this.state.noValidPolicy) {
            if (this.state.updateFund) {
            ...
            } else if (this.state.decreaseSACancelRider) {
                ...
                imgPath = 'img/popup/no-policy.svg';
            } else if (this.state.updateSundryAmount) {
        ...
        if (this.state.isCompany && (this.state.proccessType === 'SundryAmountS')) {
            msg = 'Chức năng chỉ dành cho khách hàng cá nhân. Quý khách vui lòng liên hệ văn phòng Dai-ichi Life Việt Nam để được hướng dẫn.';
            imgPath = 'img/popup/notvalid.svg';
        }
        ...
        if (this.state.lapseAboveOneYear) {
            msgPopup = 'Quý khách vui lòng liên hệ Tư vấn tài chính hoặc Văn phòng Dai-ichi Life Việt Nam gần nhất để được hướng dẫn thủ tục khôi phục hiệu lực hợp đồng';
            popupImgPath = 'img/popup/quyenloi-popup.svg';
        }
                ...
                <main className="logined" id="main-id">
                    <div className="main-warpper basic-mainflex">
                                ...
                                <div className="stepform__body" style={{paddingTop: '4px', boxShadow: 'none'}}>
                                    <div className="info">
                                        <div className="info__title" style={{marginBottom: '0px'}}>
                                            <h4>CHỌN LOẠI THÔNG TIN ĐIỀU CHỈNH</h4>
                                        </div>
                                                ...
                                                <div className="tab-wrapper">
                                                    <div className="tab">
                                                        <div className="info__title">
                                                            <h5 className='policy-change-title basic-semibold'>Chi tiết phí và sản phẩm bảo hiểm</h5>
                                                        </div>
                                                            ...
                                                            <div className="checkbox-item">
                                                                <div className="round-checkbox">
                                                                    <label className="customradio">
                                                                        <input type="checkbox"
                                                                               checked={this.state.updatePeriodicFee}
                                                                               onClick={(e) => radioPeriodicFee(e)}
                                                                               id='radioPeriodicFee'/>
                                                        ...
                                                        <div className="checkbox-warpper">
                                                            <div className="checkbox-item">
                                                                <div className="round-checkbox">
                                                                    <label className="customradio">
                                                                        <input type="checkbox"
                                                                               checked={this.state.updateSundryAmount}
                                                                               onClick={(e) => radioSundryAmount(e)}
                                                                               id='radioSundryAmount'/>
                                                                        <div className="checkmark"></div>
                                                                        <p className="big-text" style={{marginLeft: '3px'}}>
                                                                            Thay đổi phí dự tính định kỳ
                                                                        </p>
                                                                    </label>
                                                                </div>
                                                            </div>
                                                        ...
                                                        <div className="checkbox-warpper">
                                                            <div className="checkbox-item">
                                                                <div className="round-checkbox">
                                                                    <label className="customradio">
                                                                        <input type="checkbox"
                                                                               checked={this.state.decreaseSACancelRider}
                                                                               onClick={(e) => radioDecreaseSACancelRider(e)}
                                                                               id='radioPeriodicFee'/>
                                                                        <div className="checkmark"></div>
                                                                        <p className="big-text" style={{marginLeft: '3px'}}>
                                                                            Thay đổi số tiền bảo hiểm/Chấm dứt <br/> sản phẩm bổ trợ
                                                                        </p>
                                                                    </label>
                                                                </div>
                                                            </div>
                                                    ...
                                                    <div className="channel-dash-no-margin" style={{ marginLeft: '-22px', minWidth: 'calc(100% + 44px)' }}></div>
                                                    <div className="tab">
                                                        <div className="info__title">
                                                            <h5 className='policy-change-title basic-semibold'>Quỹ liên kết đơn vị</h5>
                                                        </div>
                                                        <div className="checkbox-warpper" style={{marginTop: '-17px'}}>
                                                            <div className="checkbox-item">
                                                                <div className="round-checkbox">
                                                                    <label className="customradio">
                                                                        <input type="checkbox"
                                                                               checked={this.state.updateInvestRate}
                                                                               onClick={(e) => radioInvestRate(e)}
                                                                               id='radioInvestRate'/>
                                                                        <div className="checkmark"></div>
                                                                        <p className="big-text"
                                                                           style={{marginLeft: '3px'}}>Thay đổi tỷ lệ
                                                                            đầu tư</p>
                                                                    </label>
                                                                </div>
                                                            </div>
                        ...
                        <section className="sccontract-warpper additional-information-sccontract-warpper">
                            ...
                            {this.state.isCompany && !getSession(IS_MOBILE) && (this.state.proccessType === 'SundryAmountS') &&
                            ...
                            <div className="sccontract-container" style={{backgroundColor: '#f5f3f2'}}>
                                ...
                                {this.state.stepName === FUND_STATE.CHOOSE_POLICY && !this.state.updatePeriodicFee && !this.state.decreaseSACancelRider && !this.state.updateLoan && !this.state.updateSundryAmount &&
                                    <div style={{display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
                                        <div className="bottom-text"
                                            ...
                                            <p style={{textAlign: 'justify'}}>
                                                ...
                                                style={{color: '#727272'}}>
                                                {this.state.updateReinstatement ? (
                                                    'Nếu Quý khách muốn khôi phục hợp đồng có thời gian mất hiệu lực trên 1 năm, vui lòng liên hệ Tổng đài của Dai-ichi Life Việt Nam để được hỗ trợ'
                                                ) : (
                ...
                {this.state.showThanks && (
                    this.state.updateReinstatement ? (
                        <SuccessReInstamentPopup closePopup={this.closeThanks} policyNo={this.state.polID}
                                                 screen={SCREENS.UPDATE_POLICY_INFO} fee={this.state.tempFee}/>
                    ) : (
                        (
...
export default withRouter(UpdatePolicyInfo);
