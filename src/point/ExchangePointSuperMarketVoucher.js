...
import { ACCESS_TOKEN, CLIENT_ID, USER_LOGIN, POINT, CATEGORY_IMG_FOLDER_URL, CELL_PHONE, GIFT_CART_SUPER_MARKET, TOTAL_CART_POINT, EXPIRED_MESSAGE, LINK_MENU_NAME, LINK_MENU_NAME_ID, AUTHENTICATION, FE_BASE_URL } from '../constants';
...
import { formatStrDetail, showMessage, setSession, getSession, getDeviceId } from '../util/common';
import './ExchangePointSuperMarketVoucher.css';
import { Helmet } from "react-helmet";

const ALL = 'Tất cả';
class ExchangePointSuperMarketVoucher extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loaded: false,
    ...
    this.handleSuperMarketceSubmit = this.handleSuperMarketceSubmit.bind(this);
    this.resetSelected = this.resetSelected.bind(this);
  }


  handleSuperMarketceSubmit(event) {
    event.preventDefault();
    this.addSuperMarketCards();
  }

  cpSuperMarketceGetProducts = (profile) => {
    const submitRequest = {
      jsonDataInput: {
  ...
  addSuperMarketCards = () => {
    ...
    let detail = this.state.shortDescription;
    ...
    if (indexStart >= 0 && indexEnd > 0) {
      ...
      //   position = detail.indexOf('Phiếu siêu thị');
      //   detail = [detail.slice(detail.indexOf('Phiếu siêu thị'), position + 15), typeSup, detail.slice(position + 14)].join('');
      // } else if (this.state.CategoryID === '10') {
      //   position = detail.indexOf('Phiếu quà tặng');
      //   detail = [detail.slice(detail.indexOf('Phiếu quà tặng'), position + 14), typeSup, detail.slice(position + 13)].join('');
      // }


    }
    var strDt = "{\"orderid\":\"" + 'tracking_number' + "\",\"phone\":\"" + this.state.CellPhone + "\",\"shipping\":\"shipping_fee" + "\"}" + "|" + detail + "|" + typeSup + "|" + (parseInt(this.state.MDiscount) / 1000) + "|" + this.state.Quantity + "|" + this.state.CategoryID;
    var product = {
      category: this.state.CategoryID,
      categoryName: 'Phiếu quà tặng giấy (Voucher giấy)',
      fullDescription: '',
    ...
    if (getSession(GIFT_CART_SUPER_MARKET)) {
      var giftCard = JSON.parse(getSession(GIFT_CART_SUPER_MARKET));
      ...
      if (giftCard && giftCard.length > 0) {
        ...
        setSession(GIFT_CART_SUPER_MARKET, JSON.stringify(giftCard));
      } else {
        setSession(GIFT_CART_SUPER_MARKET, JSON.stringify([product]));
      }
    } else {
      setSession(GIFT_CART_SUPER_MARKET, JSON.stringify([product]));
    }
  ...
  componentDidMount() {
    if (!this.state.selected) {
      this.cpSuperMarketceGetProducts();
    }
  }
  render() {
    ...
    const toggleFilter = () => {
      if (document.getElementById('specialfilter-id').className === 'specialfilter') {
        document.getElementById('specialfilter-id').className = 'specialfilter show';
      } else {
        document.getElementById('specialfilter-id').className = 'specialfilter';
      }
    }
    ...
    return (
      <div>
        {this.state.renderMeta &&
          <Helmet>
            <title>Phiếu quà tặng giấy (Voucher giấy) – Dai-ichi Life Việt Nam</title>
            <meta name="description" content="Cổng thông tin dành cho Khách hàng của Dai-ichi Life Việt Nam" />
            <meta name="robots" content="noindex, nofollow" />
            <meta property="og:type" content="website" />
            <meta property="og:url" content={FE_BASE_URL + "/category/13"} />
            <meta property="og:title" content="Phiếu quà tặng giấy (Voucher giấy) - Dai-ichi Life Việt Nam" />
            ...
            <link rel="canonical" href={FE_BASE_URL + "/category/13"} />
          </Helmet>
        }
        <main className="logined">
          <section className="scmarketpaper">
            <div className="breadcrums">
              ...
              <div className="breadcrums__item">
                <p>Phiếu quà tặng giấy (Voucher giấy)</p>
                <p className='breadcrums__item_arrow'>></p>
              </div>
            </div>
            <div className="container">
                ...
                <div className="useroption-card">
                        ...
                        <div className="market-card">
                          <div className="market-card__header">
                            <div className="icon-wrapper">
                              <img src={CATEGORY_IMG_FOLDER_URL + this.state.imageUrl} alt="bigc-icon" />
                            </div>
                            <p className="maxwidth">{this.state.shortDescription}</p>
                          </div>
                  ...
                  <div className="bottom-btn">
                    {getSession(CLIENT_ID) && getSession(CLIENT_ID) !== '' &&
                      <button className="btn btn-primary" onClick={this.handleSuperMarketceSubmit}>Tiếp tục</button>
                    }
              ...
              <div className="marketcards">
                <div className="marketcards__header">
                  ...
                  <div className="specialfilter" id="specialfilter-id" onClick={() => toggleFilter()}>
                ...
                <div className="marketcards__body">
                  <div className="title">
                    <h5 className="basic-semibold">Chọn phiếu quà tặng giấy (Voucher giấy)</h5>
                  </div>
                  {!this.state.loaded &&
                            ...
                            <div className="marketcard-item" key={index} onClick={() => ChooseCard(item.Price, item.Discount, item.MDiscount, 1, item.Shipping, item.strDetail, item.shortDescription, item.ProductID, item.IMAGEURL, index, item.IMAGENAME, item.CategoryID)}>
                              <div className="market-card">
                                ...
                                <div style={{ marginTop: '-15px', height: '44px', backgroundColor: '#f5f3f2' }}>
                                  {item.CategoryID === 10 &&
                                    <p style={{ fontWeight: '500', fontSize: '12px', lineHeight: '15px', color: '#727272', paddingLeft: '72px', backgroundColor: '#f5f3f2' }}>Xem thông tin quà tặng và cách sử dụng tại website www.gotit.vn</p>
                                  }
                                </div>
                                <div className="market-card__body">
                                  <div className="point">
...
export default ExchangePointSuperMarketVoucher;
