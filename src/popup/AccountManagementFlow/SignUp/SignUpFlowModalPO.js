...
import '../styles.css';
import {getPointByClientID, logoutSession, removeUndefinedFields, signup, verifyOTP} from '../../../util/APIUtils';
import {
...
const SignUpFlowModalPO = (props) => {
    ...
    const [userNameValidations, setUserNameValidations] = useState([{
        label: 'Tên đăng nhập có thể bao gồm chữ cái và chữ số, độ dài trong khoảng 5-30 ký tự.', isValid: false
    }, {label: 'Tên đăng nhập không được chứa dấu cách.', isValid: false},]);
    ...
    const popupOtpLoginSubmit = (OTP) => {
        ...
        verifyOTP(verifyOTPRequest)
            .then(response => {
                setUserNameSuggestion(response.Response.UsernameSuggestion);
                if (response.Response.Result === 'true' && response.Response.ErrLog === 'SUCCESSFUL') {
                    ...
                    // if (!isEmpty(response?.Response?.UserName)) {
                    //     setSession(USER_NAME, response.Response.UserName);
                    // }
                    if (response.Response.UsernameSuggestion) {
                        setUserName(response.Response.UsernameSuggestion);
                    ...
                    if (!isEmpty(response?.Response?.DCID)) {
    ...
    //     return [{
    //         label: 'Tên đăng nhập có thể bao gồm chữ cái và chữ số, độ dài trong khoảng 5-30 ký tự.',
    //         isValid: value.trim() === '' ? false : value.length >= 5 && value.length <= 30 && /^[a-zA-Z0-9]+$/.test(value) && /\d/.test(value),
    //     }, {
    //         label: 'Tên đăng nhập không được chứa dấu cách.', isValid: value.trim() === '' ? false : !/\s/.test(value),
    //     },];
    // };
    const getRegisterUserNameValidations = (value) => {
        return [{
    ...
    const handleInputChange = (event) => {
        ...
        if (document.getElementById('input-username')) {
            document.getElementById('input-username').className = 'input special-input-extend password-input';
        }
        ...
        signup(removeUndefinedFields(registerRequest))
            .then(response => {
                ...
                if (Response.Result === 'true' && Response.ErrLog === 'ACCOUNTCHANGEUSERNAMESUCC') {
                    const {Message, NewAPIToken, UserLogin, UsernameSuggestion} = Response;
                    ...
                    if (document.getElementById('input-username')) {
                        document.getElementById('input-username').className = 'input special-input-extend password-input validate';
                    }
                    setLoading(false);
                    // setTitle('Mật khẩu đăng nhập');
                    setDisabledConfirm(true);
    ...
    return (<>
        {isOpenPopup && <div className="signupFlowModal__overlay">
            <div className="signupFlowModal__container">
                {/* Header */}
                <div className="signupFlowModal__header">
                    {/*<div className="signupFlowModal__action-left">*/}
                    {/*    <button onClick={() => {*/}
                    {/*        if (currentStep === 0) {*/}
                    {/*            setOpenPopup(false);*/}
                    {/*        } else handlePrev();*/}
                    {/*    }}>*/}
                    {/*        <img src={iconLeft} alt="Left Icon" className="signupFlowModal__icon"/>*/}
                    {/*    </button>*/}
                    {/*</div>*/}
                ...
                <div className="signupFlowModal__content">
                    ...
                    {currentStep === 1 && <div className="username-wrapper">
                        ...
                        <div className="input-wrapper-item" style={{marginTop: 12}}>
                            <div className="input special-input-extend password-input" id='input-username'>
                                <div className="input__content">
                                    <label htmlFor="" style={{textAlign: 'left'}}>Tên đăng nhập</label>
                                    <input
                                        placeholder= {userName?`gợi ý: ${userName}`:''}
                                        value={userName}
                                        type="text"
                                        name="userName"
                                        id="new-userName"
                    ...
                    {currentStep === 2 && <div className="body">
                        ...
                        <div className="input-wrapper" style={{marginTop: 12}}>
                            ...
                            {/* <div className="error-message validate" id="pass-not-match-id">
                                <i className="icon">
                                    <img src="img/icon/warning_sign.svg" alt=""/>
                                </i>
...
export default SignUpFlowModalPO;
